...
const PET_DEX = {
  ...
  '龙渊帝君': { race:'龙', aptRange:{力量资质:[3000,3000],体质资质:[3000,3000],敏捷资质:[3000,3000],智力资质:[3000,3000]}, growthRange:[3.0,3.0], specialty:'balanced', innateSkills:['holy_radiance','fatal_blow','aura_atk_3'], desc:'【神兽】龙族至高帝君，掌控龙渊之力，万龙之主', isDivineBeast: true },
  '太古原核': { race:'史莱姆', aptRange:{力量资质:[3000,3000],体质资质:[3000,3000],敏捷资质:[3000,3000],智力资质:[3000,3000]}, growthRange:[3.0,3.0], specialty:'defense', innateSkills:['iron_wall','regen_2','aura_hp_3'], desc:'【神兽】史莱姆始祖，万物原初之核，不灭不朽', isDivineBeast: true },
  '万影至尊': { race:'哥布林', aptRange:{力量资质:[3000,3000],体质资质:[3000,3000],敏捷资质:[3000,3000],智力资质:[3000,3000]}, growthRange:[3.0,3.0], specialty:'speed', innateSkills:['sneak_atk_2','dodge_2','aura_spd_3'], desc:'【神兽】哥布林至高影王，统御万影，来去无踪', isDivineBeast: true },
  '炽天神将': { race:'天使', aptRange:{力量资质:[3000,3000],体质资质:[3000,3000],敏捷资质:[3000,3000],智力资质:[3000,3000]}, growthRange:[3.0,3.0], specialty:'balanced', innateSkills:['holy_radiance','blessing','aura_def_3'], desc:'【神兽】天使至高神将，炽天之光，守护苍生', isDivineBeast: true },
  '万木灵尊': { race:'精灵', aptRange:{力量资质:[3000,3000],体质资质:[3000,3000],敏捷资质:[3000,3000],智力资质:[3000,3000]}, growthRange:[3.0,3.0], specialty:'magic', innateSkills:['life_bloom','heal_wind','aura_regen_3'], desc:'【神兽】精灵万木之灵，生命与自然之主', isDivineBeast: true },
  '深渊魔神': { race:'恶魔', aptRange:{力量资质:[3000,3000],体质资质:[3000,3000],敏捷资质:[3000,3000],智力资质:[3000,3000]}, growthRange:[3.0,3.0], specialty:'physical', innateSkills:['doom_judge','meteor','berserk'], desc:'【神兽】恶魔深渊之主，吞噬万物的魔神', isDivineBeast: true },
  // ===== 扩展宠物（T1~T5 共 100 种） =====
...
// 神兽专属血统映射（类似 FUSION_PET_BLOODLINES）
const DIVINE_BEAST_BLOODLINES = {
  '龙渊帝君': 'dragon_emperor_blood',
  '太古原核': 'ancient_nucleus_blood',
  '万影至尊': 'shadow_lord_blood',
  '炽天神将': 'seraph_general_blood',
  '万木灵尊': 'spirit_lord_blood',
  '深渊魔神': 'abyss_demon_blood',
};
...
function generatePetBloodlineSkill(pet) {
  ...
  // 神兽专属血统
  var divineBlMap = (typeof DIVINE_BEAST_BLOODLINES !== 'undefined') ? DIVINE_BEAST_BLOODLINES : {};
  if (divineBlMap[pet.name]) {
    var divineSkill = BLOODLINE_SKILLS.find(function(b) { return b.id === divineBlMap[pet.name]; });
    if (divineSkill) {
      var divineCopy = Object.assign({}, divineSkill);
      divineCopy.mechanics = _getMechanics(pet.name);
      return divineCopy;
    }
    return divineSkill;
  }
  // 已植入血统珠
  // 需求1重构：血统珠不再替换为种族通用血统，而是强化宠物自身专属血统
  // 品质乘数决定血统效果强度，sourcePetName保留用于来源展示
  if (pet.bloodlineOrb && pet.bloodlineOrb.sourcePetName) {
    ...
    // 需求1：使用宠物自身专属血统（非种族通用血统），品质乘数强化效果
    var orbDex = getPetDex(pet.name);
    var orbSpecialty = orbDex.specialty || 'balanced';
    var orbStage = pet.advanceStage || 0;
    var orbStageMult = orbStage === 2 ? 2.0 : orbStage === 1 ? 1.5 : 1.0;
    var orbCustomEffects = (typeof PET_BLOODLINE_EFFECTS !== 'undefined' && PET_BLOODLINE_EFFECTS[pet.name]) || null;
    var orbBaseEffects = orbCustomEffects || BLOODLINE_EFFECT_PRESETS[orbSpecialty] || BLOODLINE_EFFECT_PRESETS.balanced;
    var orbEffects = {};
    Object.keys(orbBaseEffects).forEach(function(k) {
      orbEffects[k] = Math.round(orbBaseEffects[k] * orbStageMult * orbMult * 100) / 100;
    });
    var orbBlId = 'bl_' + pet.name + '_orb';
    var orbTier = getPetTier(pet.name);
    var orbDexEntry = PET_BLOODLINE_DEX ? PET_BLOODLINE_DEX[pet.name] : null;
    var orbBlName = orbDexEntry ? orbDexEntry.name : (pet.name + '之血');
    var orbSpecLabel = { physical: '物理', magic: '法术', defense: '防御', speed: '速度', balanced: '均衡' }[orbSpecialty] || '均衡';
    var orbDesc;
    if (orbDexEntry) {
      orbDesc = '【' + orbSpecLabel + '系·植入强化】' + orbDexEntry.desc;
    } else {
      var orbEffectStr = formatBloodlineEffects(orbEffects);
      orbDesc = '【' + orbSpecLabel + '系血统·植入强化】' + orbEffectStr;
    }
    var orbMechanics = _getMechanics(pet.name);
    return {
      id: orbBlId, name: orbBlName, type: 'bloodline', race: orbDex.race, tier: orbTier,
      desc: orbDesc, effects: orbEffects, specialty: orbSpecialty, mechanics: orbMechanics,
      fromBloodlineOrb: true, quality: orbQuality, sourcePetName: pet.bloodlineOrb.sourcePetName
    };
  }
// 需求4：基于宠物名生成独特血统
...
const ITEM_CONFIG = {
  ...
  // ===== 神兽精华（新增） =====
  divine_essence:   { name:'神兽精华', desc:'蕴含神兽之力的精华，集齐99个可随机兑换1只神兽。不可交易、分解、炼化', tier:6, valueLevel:'S', dropWeight:1, tradeable:false, decomposable:false, refinable:false, stackMax:999, shopPrice:50, currency:'diamond', sources:['商城','藏宝图(史诗/传奇)','挖秘藏(黄金宝藏)','兑换码'] },
