...
function getPetStats(pet) {
  ...
  return {
    ...
    速度: safeNum(finalSpd),
    体质: safeNum(体质), 力量: safeNum(力量), 敏捷: safeNum(敏捷), 智慧: safeNum(智慧),
    法力: safeNum(finalMp), 智力: safeNum(finalInt), 力量_: safeNum(力量), 体质_: safeNum(体质),
    critRate: safeNum((cb.critRate || 0) + setCritRate),
    dodgeRate: safeNum((cb.dodgeRate || 0) + setDodgeRate),
    hitRate: safeNum((cb.hitRate || 0) + formHitRate),
    critDmg: safeNum((cb.critDmg || 0) + setCritDmg),
    skillTrigger: safeNum(cb.skillTrigger || 0),
    skillDmg: safeNum(cb.skillDmg || 0),
    vampPct: safeNum((cb.vampPct || 0) + (runeBonus ? (runeBonus.pctAdditions.vampPct || 0) : 0)),
    dmgReduce: safeNum((cb.dmgReduce || 0) + formDmgReduce),
    regenPct: safeNum(setRegenPct),
    magicDmgPct: safeNum(setMagicDmgPct + (runeBonus ? (runeBonus.pctAdditions.skillDmg || 0) : 0)),
...
function generateDivineBeast() {
  ...
  var growth = 3.0;
  // 天生技能——使用公共函数（仅获取图鉴定义技能）
  var innateSkills = generateInnateSkills(name, (dex.innateSkills ? dex.innateSkills.length : 1), dex);
  var newPet = {
    name: name,
    race: race,
