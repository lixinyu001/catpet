...
(function init() {
  try {
    ...
    // Bug 1 修复：绝不覆盖存档！loadGame() 已完成存档加载，G 中已有玩家数据
    // 仅确保基本字段存在，尝试继续渲染
    try {
      if (typeof _ensureSaveStateFields === 'function') _ensureSaveStateFields();
      else if (!G || !G.player) G = JSON.parse(JSON.stringify(DEFAULT_STATE));
      // 不调用 saveGame()，避免用当前（可能不完整的）状态覆盖存档
      render();
    } catch(e2) {
      console.error('Init recovery failed:', e2);
      // Bug 1 修复：不删除存档！只显示错误页面
      window.__GAME_LOADED__ = false;
      var errMsg2 = (e && e.message) ? e.message : String(e || '未知错误');
      document.getElementById('app').innerHTML = '<div style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:#0a0a14;color:#e2e8f0;font-family:-apple-system,sans-serif;"><div style="text-align:center;padding:2rem;max-width:420px;"><p style="color:#ef4444;font-size:1.125rem;margin-bottom:0.5rem;">游戏初始化失败</p><p style="color:#94a3b8;font-size:0.875rem;margin-bottom:1rem;">' + errMsg2 + '</p><p style="color:#f59e0b;font-size:0.8rem;margin-bottom:0.75rem;">存档已保留，刷新页面可重试</p><button onclick="location.reload()" style="background:linear-gradient(135deg,#3b82f6,#2563eb);color:#fff;border:1px solid #60a5fa;padding:0.75rem 2rem;border-radius:0.5rem;cursor:pointer;font-size:0.95rem;font-weight:700;margin-bottom:0.75rem;">刷新重试</button><br><button onclick="window.clearSaveAndReload()" style="background:linear-gradient(135deg,#ef4444,#dc2626);color:#fff;border:1px solid #f87171;padding:0.5rem 1.5rem;border-radius:0.5rem;cursor:pointer;font-size:0.875rem;margin-top:0.5rem;">清除存档重新开始</button></div></div>';
    }
  }
})();
