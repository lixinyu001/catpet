...
function renderPetScreen() {
  // v3.x 需求1.1：进入宠物页面时自动刷新所有宠物属性与战力，确保展示最新数据
  if (typeof _autoRefreshAllPetStats === 'function') _autoRefreshAllPetStats();
...
function renderPetDetailModal() {
  ...
  <div class="modal-overlay" onclick="closePetDetail()">
    <div class="modal-content scrollbar-thin" onclick="event.stopPropagation()" style="max-width:560px;max-height:85vh;">
      ...
      ${(() => {
        // v3.x 需求1.2：移除自动比例加点，替换为单项属性一键加满
        ...
        var allBtnClass = freePts <= 0 ? 'bg-gray-700 text-gray-500 cursor-not-allowed' : 'bg-yellow-700 hover:bg-yellow-600 border border-yellow-500';
        // 五维属性加点区域：每维含当前值、输入框、+1按钮、全部加满按钮
        var batchHtml = ['力量','敏捷','体质','耐力','魔力'].map(function(k) {
          return '<div class="bg-panel rounded-lg p-2 text-center flex flex-col items-center gap-1">' +
            ...
            '<div class="flex gap-1">' +
              ...
              '<button onclick="_allocateAllAttrPoints(\'' + pet.id + '\',\'' + k + '\')" class="' + allBtnClass + ' rounded px-2 py-0.5 text-xs"' + disabled + ' title="将剩余自由属性点全部分配到' + k + '">全部</button>' +
            '</div>' +
            '</div>';
        }).join('');
        return '<div class="mb-4 bg-game-dark rounded-lg p-3 border border-yellow-600/30">' +
          ...
          '</div>' +
          '<p class="text-xs text-secondary mt-2">每升1级获得10点属性点（5固定+5自由），点击「全部」将剩余自由点一次性分配到对应属性</p>' +
          '</div>';
      })()}
            ...
            return pageItems.map(item => {
              ...
              <div class="bg-panel border border-game rounded-xl p-4 text-center flex flex-col">
                ...
                <div class="flex items-center justify-center gap-2 my-2">
                  ...
                  <input type="number" min="1" max="9999" value="${qty}" class="w-14 text-center font-bold text-sm bg-game-dark border border-game rounded px-1 py-0.5" onchange="event.stopPropagation();setShopQty('${item.id}',Math.max(1,parseInt(this.value)||1));renderShopOnly()" onclick="event.stopPropagation()">
                  <button class="w-7 h-7 rounded border border-game text-sm flex items-center justify-center hover:border-purple-500" onclick="event.stopPropagation();setShopQty('${item.id}',getShopQty('${item.id}')+1);renderShopOnly()">+</button>
                  <button class="text-xs px-2 py-0.5 rounded border border-yellow-600 text-yellow-400 hover:bg-yellow-900/40" onclick="event.stopPropagation();setShopQty('${item.id}',Math.max(1,getMaxShopQty(${item.price},'${item.currency || 'gold'}')));renderShopOnly()" title="填入当前货币可购买的最大数量">最大</button>
                </div>
