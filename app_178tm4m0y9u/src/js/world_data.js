...
function generateChestContents(rarity) {
  ...
  var pools = {
    white: [
      { w: 20, quality: 'mid',  gen: function(){ return { type: 'item', id: 'hatch_boost', amount: 1, name: '孵化加速器' }; } },
      { w: 18, quality: 'low',  gen: function(){ return _genEquip('white'); } },
...
var EVOLUTION_FOREST_CONFIG = {
  id: 'evolution_forest',
  name: '符文洞窟',
  desc: '探索神秘符文洞窟，获取符文材料与成品符文等丰厚奖励',
  dailyLimit: 50,
  minLevel: 20,
  layers: [
  ...
  rewards: [
    // layer 1: 低级符文材料×2~4, 10%概率中级材料×1, 10%掉成品符文(原5%)
    { layer: 1, runeLow: [2, 4], runeMid: { chance: 0.10, count: 1 }, runeHigh: null, runeDropRate: 0.10, gold: [500, 1500], exp: [200, 500] },
    // layer 2: 低级符文材料×3~6, 30%概率中级材料×1, 16%掉成品符文(原8%)
    { layer: 2, runeLow: [3, 6], runeMid: { chance: 0.30, count: 1 }, runeHigh: null, runeDropRate: 0.16, gold: [1000, 3000], exp: [500, 1000] },
    // layer 3: 必掉中级符文材料×1~2, 5%概率高级材料×1, 24%掉成品符文(原12%)
    { layer: 3, runeLow: [5, 8], runeMid: { chance: 1.00, count: 2 }, runeHigh: { chance: 0.05, count: 1 }, runeDropRate: 0.24, gold: [2000, 5000], exp: [1000, 2000] },
    // layer 4: 必掉中级符文材料×2~3, 20%概率高级材料×1, 36%掉成品符文(原18%)
    { layer: 4, runeLow: [8, 12], runeMid: { chance: 1.00, count: 3 }, runeHigh: { chance: 0.20, count: 1 }, runeDropRate: 0.36, gold: [4000, 8000], exp: [2000, 4000] },
    // layer 5: 必掉高级符文材料×1~2, 50%概率额外高级×1, 60%掉成品符文(原30%)
    { layer: 5, runeLow: [10, 15], runeMid: { chance: 1.00, count: 5 }, runeHigh: { chance: 0.80, count: 2 }, runeDropRate: 0.60, gold: [8000, 15000], exp: [4000, 8000] },
  ],
};
