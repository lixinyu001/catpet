...
// v2.11.0 需求4.2：解锁等级下调至20级，每5级一档（20/25/30...），基础属性+20%，阶段增幅×1.5
//   stage = max(1, floor((level - 20) / 5) + 1)，stage1=lv20-24, stage2=lv25-29, ...
//   baseVal = STAGE1_BASE[slot] × 1.2(基础强化) × (1.5 ^ (stage - 1)) × (1 + rarityIdx × 0.3)
function generatePetEquip(rarity, slot, level) {
  ...
  // 每升一档（+5级）基础属性 ×1.5（阶段增幅50%）
  var stage = Math.max(1, Math.floor((level - 20) / 5) + 1);
  var stageMult = Math.pow(1.5, stage - 1);
...
function syncPetEquip(petId, slot) {
  ...
  var stageMult = Math.pow(1.5, newStage - 1);
  var baseBoost = 1.2;
  var rarityMult = 1 + rarityIdx * 0.3;
  switch (equip.slot) {
    case 'attack':
