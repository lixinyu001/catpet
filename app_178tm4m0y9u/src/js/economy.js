...
function enterEvolutionForest(layer) {
  ...
  // 需求3.1：使用弹窗战斗系统替代 spawnMonsterDirect，与押镖/宠物秘境界面一致
  var cp = Math.floor(getPlayerCombatPower());
  // 怪物强度：基于层数等级和配置的 hpMult/atkMult 综合计算
  var monsterPower = lv * 800 * layerConfig.hpMult * 0.2 * (1 + (layer - 1) * 0.15);
  // 基础胜率 65% + 战力差调整，高层数难度递增
  var winChance = 0.65 + (cp - monsterPower) / Math.max(monsterPower, 1) * 0.5;
  winChance = Math.max(0.10, Math.min(0.95, winChance));

  startActivityBattleModal({
    type: 'evolution_forest',
    stage: layer,
    enemyName: layerConfig.name + ' · ' + layerConfig.monsterName,
    enemyIcon: layerConfig.icon,
    enemyLv: lv,
    monsterPower: monsterPower,
    playerCp: cp,
    winChance: winChance,
    onComplete: function(success) { _finishEvolutionForestBattle(success, layer); }
  });

  return { ok: true };
}

// 符文洞窟弹窗战斗结算（原进化森林，v2.8.0重构）
// 挑战成功发放奖励并扣除次数，挑战失败不扣除次数
function _finishEvolutionForestBattle(success, layer) {
  if (!success) {
    showToast('💀 符文洞窟第' + layer + '层挑战失败，不扣除次数', 'error');
    if (typeof addActivityLog === 'function') addActivityLog('evolution_forest', '第' + layer + '层挑战失败（不扣除次数）', 'fail');
    saveGame();
    if (typeof render === 'function') render();
    return;
  }
  // 胜利结算：调用 settleEvolutionForest 发放奖励并扣除次数
  var result = settleEvolutionForest(layer);
  if (result.ok) {
    var msg = '🔮 符文洞窟第' + layer + '层挑战成功！';
    if (result.rewards) {
      if (result.rewards.gold) msg += ' 金币+' + result.rewards.gold;
      if (result.rewards.exp) msg += ' 经验+' + result.rewards.exp;
      if (result.rewards.items && result.rewards.items.length > 0) {
        result.rewards.items.forEach(function(it) {
...
function startPetCaveBattle() {
  ...
  // 需求3.1：使用弹窗战斗系统替代 enterSpecialDungeon
  var cp = Math.floor(getPlayerCombatPower());
  var monsterLv = G.player.level;
  // 5波秘境守卫，整体强度参考原副本公式
  var monsterPower = monsterLv * 800 * 5 * 0.8;
  var winChance = 0.65 + (cp - monsterPower) / Math.max(monsterPower, 1) * 0.5;
  winChance = Math.max(0.15, Math.min(0.95, winChance));
  ...
  startActivityBattleModal({
    ...
    playerCp: cp,
    winChance: winChance,
    onComplete: function(success) { _finishPetCaveBattle(success); }
  });
}

// 宠物秘境战斗结算
function _finishPetCaveBattle(success) {
  if (!success) {
    showToast('💀 宠物秘境挑战失败', 'error');
    if (typeof addActivityLog === 'function') addActivityLog('petcave', '挑战失败', 'fail');
    saveGame();
    if (typeof render === 'function') render();
    return;
  }
  // 通关奖励：与原副本系统一致的宠物装备掉落逻辑
  var waveCount = 5; // 固定5波
  var peRarityPool;
  ...
  var rewardMsg = '获得 ' + PET_EQUIP_RARITY_NAMES[peRarityIdx] + ' ' + peEquip.name;
  // 30%概率额外掉落材料
  if (Math.random() < 0.30) {
    var matGradePool = waveCount >= 5 ? ['mid','high'] : waveCount >= 3 ? ['low','mid'] : ['low'];
    ...
    rewardMsg += '，' + PET_EQUIP_MATERIAL_NAMES[crystalKey] + ' x' + matCount + '，' + PET_EQUIP_MATERIAL_NAMES[warBookKey] + ' x' + matCount;
  }
  showToast('🐾 宠物秘境通关！' + rewardMsg, 'success');
  if (typeof addActivityLog === 'function') addActivityLog('petcave', '通关宠物秘境（5波），' + rewardMsg, 'win');
  saveGame();
  if (typeof render === 'function') render();
}
...
function startActivityBattleModal(opts) {
  ...
  // v3.x 需求4.1：修复活动战斗胜负判定逻辑——仅当玩家方所有上阵宠物全部阵亡时，才判定为挑战失败
  // 敌方HP归零 → 胜利；我方HP归零 → 失败；回合耗尽且双方均未全灭 → 玩家胜利（敌方未能在回合限制内全灭玩家）
  var success;
  if (enemyHp <= 0) {
    success = true;
    enemyHp = 0;
