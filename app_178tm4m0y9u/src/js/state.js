...
function _doSave() {
  try {
    ...
    // Bug 1 修复：存储空间不足时不删除存档，改为提示用户
    if (e.name === 'QuotaExceededError') {
      Logger.error('[Save]', '存储空间不足！');
      // 尝试清理非关键 localStorage 项后重试一次
      var _retryOK = false;
      try {
        // 清理可能的其他缓存数据，但保留游戏存档
        var _toRemove = [];
        for (var i = 0; i < localStorage.length; i++) {
          ...
          if (k && k !== SAVE_KEY && k.indexOf('shadow_era') !== 0) {
            _toRemove.push(k);
          }
        }
        _toRemove.forEach(function(k) { try { localStorage.removeItem(k); } catch(e3) {} });
        // 重试保存
        localStorage.setItem(SAVE_KEY, data);
        _saveDirty = false;
        _lastSaveTime = Date.now();
        ...
        Logger.error('[Save]', '清理后重试仍失败: ' + (e2.message || e2));
      }
      // 向用户显示明确提示（非静默丢失）
      setTimeout(function() {
        if (typeof showToast === 'function') {
          showToast('⚠️ 存储空间不足，存档可能未保存！请清理浏览器缓存或导出存档。', 'error');
        }
      }, 100);
      // 保持 _saveDirty = true，下次 saveGame 时会重试
      if (!_retryOK) _saveDirty = true;
    }
  }
}

function loadGame() {
  // Bug 1 修复：分离 JSON 解析和数据迁移，避免迁移错误导致存档被删除
  var saved = null;
  var parsed = null;

  // 阶段1：JSON 解析（仅存档损坏才走 catch）
  try {
    ...
    console.error('[Load] 存档解析失败:', e);
    // Bug 1 修复：绝不删除存档！保留原始数据以便后续恢复
    G = JSON.parse(JSON.stringify(DEFAULT_STATE));
    setTimeout(function() {
      if (typeof showToast === 'function') {
        showToast('⚠️ 存档数据解析异常，已使用默认数据启动。原始存档已保留。', 'error');
      }
    }, 500);
    return;
  }

  if (!parsed) return; // 无存档，使用默认 G

  // 阶段2：数据合并与迁移（迁移失败不删除存档）
  try {
...
if (G.mainQuestCycleCount === undefined) G.mainQuestCycleCount = 0;
    ...
    console.error('[Load] 存档迁移失败，尝试安全恢复:', e);
    // Bug 1 修复：迁移失败时绝不删除存档！确保基本字段存在避免渲染崩溃
    _ensureSaveStateFields();
    setTimeout(function() {
      if (typeof showToast === 'function') {
        showToast('⚠️ 存档数据迁移部分异常，已尝试恢复。如遇问题请联系支持。', 'error');
      }
    }, 500);
  }
}

// Bug 1 修复：存档字段安全初始化（迁移失败时的兜底，确保 G 可用）
function _ensureSaveStateFields() {
  if (!G.player) G.player = JSON.parse(JSON.stringify(DEFAULT_STATE.player));
  if (!G.player.activeTeam || !Array.isArray(G.player.activeTeam)) G.player.activeTeam = [null, null, null];
  if (!G.player.equipment) G.player.equipment = { weapon: null, helmet: null, armor: null, pants: null, gloves: null, shoes: null };
  if (!G.player.forgeLevels) G.player.forgeLevels = { weapon: 0, helmet: 0, armor: 0, pants: 0, gloves: 0, shoes: 0 };
  ...
  if (!G.talents || typeof G.talents !== 'object') G.talents = { origin: 1 };
  if (G.talentPointsEarned === undefined) G.talentPointsEarned = G.player.level || 1;
  if (!G.runeBag || !Array.isArray(G.runeBag)) G.runeBag = [];
  if (!G.equipmentBag || !Array.isArray(G.equipmentBag)) G.equipmentBag = [];
  if (!G.gemBag || !Array.isArray(G.gemBag)) G.gemBag = [];
  if (!G.gems || typeof G.gems !== 'object') G.gems = { weapon: null, helmet: null, armor: null, pants: null, gloves: null, shoes: null };
  if (!G.marketListings || !Array.isArray(G.marketListings)) G.marketListings = [];
  if (!G.formations || typeof G.formations !== 'object') G.formations = {};
  if (!G.samsara || typeof G.samsara !== 'object') G.samsara = { currentFloor: 0, maxFloorCleared: 0, reincarnationPoints: 0, inChallenge: false, divinePowers: {} };
  if (!G.statistics) G.statistics = { totalBattles: 0, totalHatches: 0, totalFusions: 0, totalDungeons: 0 };
  if (!G.petEquipBag || !Array.isArray(G.petEquipBag)) G.petEquipBag = [];
  if (!G.petEquipMaterials) G.petEquipMaterials = { mystic_crystal_low: 0, mystic_crystal_mid: 0, mystic_crystal_high: 0, war_book_low: 0, war_book_mid: 0, war_book_high: 0 };
  if (!G.runeMaterials) G.runeMaterials = { ancient_rune_low: 0, ancient_rune_mid: 0, ancient_rune_high: 0 };
...
function createStarterPet(name, race, rarity) {
  // 需求5.1：新手礼包宠物属性统一固化——成长值固定1.0，资质全部固定1200
  const growth = 1.0;
