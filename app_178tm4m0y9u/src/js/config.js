...
// stage: 1 = T3(mid 进阶), 2 = T5(top 进阶)
// 需求2：进阶系数统一为1.30，图鉴数值下调以保证平衡
// T3类（stage=1）资质×1.2 成长+0.3（天生较强，进阶一次即T3）
// T5类（stage=2）资质×1.5 成长+0.7（最终形态，最强）
function getAdvancedPetDex(baseName, stage) {
  ...
  // 需求2：图鉴资质倍率下调（1.4→1.2, 1.8→1.5），配合 advancePet 1.30 系数
  var mult = stage === 1 ? 1.2 : 1.5;
  var aptRange = {};
  Object.keys(baseDex.aptRange).forEach(function(k) {
    var range = baseDex.aptRange[k];
    aptRange[k] = [Math.min(2800, Math.floor(range[0] * mult)), Math.min(3000, Math.floor(range[1] * mult))];
  });
  var gRange = baseDex.growthRange;
  // 需求2：成长加成下调（+0.6→+0.3, +1.2→+0.7）
  var growthRange = stage === 1
    ? [gRange[0] + 0.3, gRange[1] + 0.3]   // T3: 成长 +0.3（天生较强）
    : [gRange[0] + 0.7, gRange[1] + 0.7];  // T5: 成长 +0.7（最终形态）
  return {
    race: baseDex.race,
    aptRange: aptRange,
    growthRange: growthRange,
    specialty: baseDex.specialty,
    innateSkills: baseDex.innateSkills.slice(), // 技能不变，进阶阶段通过 advanceStage 强化
...
function getPetMaxSkills(name) {
  // 按品阶决定满技能格：T1=4, T2=4, T3=5, T4=5, T5=6（参考梦幻西游）
...
function generatePetBloodlineSkill(pet) {
  ...
  var preset = BLOODLINE_EFFECT_PRESETS[specialty] || BLOODLINE_EFFECT_PRESETS.balanced;
  var stage = pet.advanceStage || 0;
  var mult = stage === 2 ? 2.0 : stage === 1 ? 1.5 : 1.0;
  var effects = {};
  Object.keys(preset).forEach(function(k) {
    effects[k] = Math.round(preset[k] * mult * 100) / 100;
  });
...
function getItemQualityById(id) {
  ...
  var midItems = ['hatch_boost', 'fusion_stone', 'exp_ticket', 'gold_ticket', 'egg_ticket', 'skill_ticket', 'forge_ticket', 'map_ticket', 'gem_ticket', 'pet_ticket', 'guiyuan_pill', 'forge_stone_mid'];
...
const DUNGEONS = [
  { id: 'exp_cave', name: '经验洞穴', type: 'special', minLv: 30, desc: '大量经验奖励的特殊副本', ticketItem: 'exp_ticket' },
  { id: 'gold_mine', name: '黄金矿洞', type: 'special', minLv: 20, desc: '大量金币奖励的特殊副本', ticketItem: 'gold_ticket' },
  { id: 'egg_forest', name: '蛋之森林', type: 'special', minLv: 15, desc: '掉落宠物蛋的特殊副本', ticketItem: 'egg_ticket' },
  // 需求6：技能秘境已移到活动页面（赚取技能书）
  { id: 'forge_mine', name: '强化石矿脉', type: 'special', minLv: 35, desc: '掉落各级强化石的特殊副本', ticketItem: 'forge_ticket' },
  { id: 'treasure_ruin', name: '藏宝遗迹', type: 'special', minLv: 40, desc: '掉落藏宝图的特殊副本', ticketItem: 'map_ticket' },
  { id: 'gem_cavern', name: '宝石秘洞', type: 'special', minLv: 30, desc: '掉落各类宝石的特殊副本', ticketItem: 'gem_ticket' },
  // 需求5：血统副本
  { id: 'bloodline_dungeon', name: '血统副本', type: 'special', minLv: 25, desc: '挑战血统守护者，掉落血统珠道具', ticketItem: 'blood_dungeon_ticket' },
  // 需求12：宠物秘境已移到活动页面
];
...
// 需求3：按目前设定更新每日任务（覆盖当前所有活动系统）
const DAILY_TASKS = [
  { id: 'login', name: '每日登录', desc: '登录游戏', reward: { diamond: 10 }, progress: 1, target: 1 },
  { id: 'battle_10', name: '战斗达人', desc: '完成10场战斗', reward: { diamond: 20 }, progress: 10, target: 10 },
  { id: 'hatch_egg', name: '孵化师', desc: '孵化1个宠物蛋', reward: { diamond: 15 }, progress: 1, target: 1 },
  { id: 'fuse_pet', name: '融合实验', desc: '进行1次宠物融合', reward: { diamond: 25 }, progress: 1, target: 1 },
  { id: 'dungeon_run', name: '副本勇士', desc: '完成1次副本', reward: { diamond: 30 }, progress: 1, target: 1 },
  { id: 'market_trade', name: '商人', desc: '在市场进行1次交易', reward: { diamond: 10 }, progress: 1, target: 1 },
  { id: 'draw_10', name: '抽蛋爱好者', desc: '抽蛋10次', reward: { diamond: 15 }, progress: 10, target: 10 },
  { id: 'tower_5', name: '爬塔先锋', desc: '爬塔5层', reward: { diamond: 25 }, progress: 5, target: 5 },
  { id: 'forge_1', name: '锻造匠人', desc: '强化装备1次', reward: { diamond: 15 }, progress: 1, target: 1 },
  { id: 'arena_3', name: '竞技斗士', desc: '竞技场挑战3次', reward: { diamond: 20 }, progress: 3, target: 3 },
  { id: 'treasure_hunt', name: '寻宝猎人', desc: '完成1次打宝图活动', reward: { diamond: 20 }, progress: 1, target: 1 },
  { id: 'gold_earn_5k', name: '财富日赚', desc: '当日累计获得5000金币', reward: { diamond: 10 }, progress: 5000, target: 5000 },
  { id: 'skill_learn', name: '技能研习', desc: '学习1本技能书', reward: { diamond: 15 }, progress: 1, target: 1 },
  // 需求3：新增活动每日任务
  { id: 'formation_escort', name: '押镖勇士', desc: '参与1次押镖活动', reward: { diamond: 20 }, progress: 1, target: 1 },
  { id: 'skillbook_hunt', name: '技能秘境探索', desc: '参与1次技能秘境挑战', reward: { diamond: 20 }, progress: 1, target: 1 },
  { id: 'petcave_1', name: '宠物秘境', desc: '参与1次宠物秘境挑战', reward: { diamond: 25 }, progress: 1, target: 1 },
  { id: 'advance_trial', name: '进阶试炼', desc: '参与1次宠物进阶试炼', reward: { diamond: 25 }, progress: 1, target: 1 },
  { id: 'petequip_craft', name: '宠物装备工匠', desc: '打造1件宠物装备', reward: { diamond: 15 }, progress: 1, target: 1 },
  { id: 'petequip_decomp', name: '宠物装备回收', desc: '分解1件宠物装备', reward: { diamond: 10 }, progress: 1, target: 1 },
  { id: 'socket_drill', name: '装备打孔', desc: '进行1次装备打孔', reward: { diamond: 15 }, progress: 1, target: 1 },
  { id: 'bloodline_extract', name: '血统提取', desc: '进行1次血统提取', reward: { diamond: 20 }, progress: 1, target: 1 },
  { id: 'pet_advance', name: '宠物进阶', desc: '完成1次宠物进阶', reward: { diamond: 30 }, progress: 1, target: 1 },
  // 派遣奇遇系统
  { id: 'dispatch_finish', name: '派遣归来', desc: '完成1次派遣奇遇', reward: { diamond: 20 }, progress: 1, target: 1 },
];
...
// 宠物装备品质：稀有1词条 / 史诗2词条 / 传说3词条 / 神话4词条
const PET_EQUIP_RARITIES = ['rare', 'epic', 'legend', 'mythic'];
const PET_EQUIP_RARITY_NAMES = ['稀有', '史诗', '传说', '神话'];
const PET_EQUIP_RARITY_COLORS = ['#3b82f6', '#a855f7', '#fb923c', '#ef4444'];
const PET_EQUIP_AFFIX_COUNT = [1, 2, 3, 4];
...
// 产出品质概率（按打造等级）
// 低级打造：100%稀有
// 中级打造：60%稀有+30%史诗+10%传说
// 高级打造：20%史诗+40%传说+40%神话
const PET_EQUIP_CRAFT_RATES = {
  low:  { rare: 1.00 },
  mid:  { rare: 0.60, epic: 0.30, legend: 0.10 },
  high: { epic: 0.20, legend: 0.40, mythic: 0.40 },
};
