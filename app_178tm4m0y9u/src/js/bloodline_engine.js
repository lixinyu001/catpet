...
var PET_BLOODLINE_MECHANICS = {
  ...
  // ===== 神兽（6只）=====
  '龙渊帝君': [
    { id:'dragon_edict_debuff', trigger:'onBattleStart', target:'enemyAll', action:'applyStatus', params:{ status:'atkReduce', atkPct:-0.10, duration:2 } },
    { id:'dragon_press_atk', trigger:'onAttackHit', target:'target', action:'applyStatus', params:{ status:'atkReduce', atkPct:-0.05, duration:2 } }
  ],
  '太古原核': [
    { id:'nucleus_revive', trigger:'onFatalDamage', maxTrigger:1, target:'self', action:'revive', params:{ hpPct:0.50, clearDebuff:true } },
    { id:'nucleus_regen', trigger:'onTurnEnd', target:'self', action:'healPct', params:{ pct:0.15, base:'maxHp' } }
  ],
  '万影至尊': [
    { id:'shadow_counter', trigger:'onDodge', maxTrigger:3, target:'attacker', action:'counter', params:{ counterPct:1.50, dmgType:'shadow' } }
  ],
  '炽天神将': [
    { id:'radiant_revive', trigger:'onFatalDamage', maxTrigger:1, target:'self', action:'revive', params:{ hpPct:0.40, clearDebuff:true } },
    { id:'radiant_regen', trigger:'onTurnEnd', target:'self', action:'healPct', params:{ pct:0.10, base:'maxHp' } }
  ],
  '万木灵尊': [
    { id:'life_regen_all', trigger:'onTurnEnd', target:'allyAll', action:'healPct', params:{ pct:0.05, base:'maxHp' } },
    { id:'life_heal_boost', trigger:'onHealCast', target:'self', action:'modifyStat', params:{ stat:'healBoost', value:0.30, mode:'add', applyTo:'currentHeal' } }
  ],
  '深渊魔神': [
    { id:'abyss_erode', trigger:'onAttackHit', target:'target', action:'applyStatus', params:{ status:'defReduce', defPct:-0.15, duration:2 } },
    { id:'abyss_devour', trigger:'onKill', target:'self', action:'applyStatus', params:{ status:'atkBonus', atkPctPerStack:0.05, maxStack:99, stack:1 } }
  ],
  // ===== 可进阶宠物 T1（12只）=====
  '星光雏': [
    { id:'star_save', trigger:'onFatalDamage', maxTrigger:1, target:'self', action:'setHp', params:{ value:'1' } },
    { id:'star_dodge', trigger:'onFatalDamage', maxTrigger:1, target:'self', action:'modifyStat', params:{ stat:'dodgeRate', value:0.15, mode:'add' } }
  ],
  '烈焰幼龙': [
    { id:'fire_boost', trigger:'onAttackHit', condition:{ selfHpBelow:0.50 }, target:'self', action:'modifyNextAttack', params:{ stat:'dmgPct', value:0.30, mode:'add', consume:true } },
    { id:'fire_stack', trigger:'onAttackHit', condition:{ selfHpBelow:0.50 }, target:'self', action:'applyStatus', params:{ status:'fireDmgBonus', dmgPctPerStack:0.02, maxStack:5, stack:1 } }
  ...
  '寒冰幼龟': [
    { id:'frostbite_counter', trigger:'onHit', chance:0.10, target:'attacker', action:'applyStatus', params:{ status:'frostbite', stack:1, maxStack:3, duration:2, spdPct:-0.10 } },
    { id:'frost_armor', trigger:'onTurnStart', target:'self', action:'applyStatus', params:{ status:'defBonus', defPct:0.03, maxStack:5, stack:1, duration:999 } }
  ],
  '暗影幼狐': [
    { id:'shadow_ambush', trigger:'onFirstAttack', target:'self', action:'modifyNextAttack', params:{ stat:'dmgPct', value:0.20, mode:'add', consume:true } },
    { id:'shadow_no_counter', trigger:'onFirstAttack', target:'target', action:'applyStatus', params:{ status:'noCounter', duration:1 } },
    { id:'dodge_crit', trigger:'onDodge', target:'self', action:'modifyNextAttack', params:{ stat:'critRate', value:1.0, mode:'add', consume:true } }
  ],
  '雷霆幼豹': [
    { id:'thunder_charge', trigger:'onAttackHit', chance:0.30, condition:{ isFirstAttack:true, resetPerTurn:true }, target:'target', action:'extraAttack', params:{ dmgPct:0.15, dmgType:'lightning', powerAttr:'atk' } },
    { id:'thunder_speed', trigger:'onAttackHit', chance:0.30, condition:{ isFirstAttack:true, resetPerTurn:true }, target:'self', action:'applyStatus', params:{ status:'spdBonus', spdPct:0.10, duration:1 } }
  ],
  '圣光幼灵': [
    { id:'extra_heal', trigger:'onHealCast', chance:0.20, target:'allyRandom1', action:'heal', params:{ healPct:0.50, base:'lastHealAmount' } }
  ],
  '翠叶幼精': [
    { id:'leaf_regen', trigger:'onTurnEnd', target:'self', action:'healPct', params:{ pct:0.03, base:'maxHp' } },
    { id:'leaf_dew', trigger:'onSkillCast', chance:0.15, target:'allyLowestHp', action:'applyStatus', params:{ status:'flowerDew', hotPct:0.03, duration:2 } }
  ],
  '铁甲幼蟹': [
    { id:'crab_block', trigger:'onHit', chance:0.15, target:'self', action:'forceBlock', params:{} },
    { id:'crab_pierce', trigger:'onHit', chance:0.15, target:'self', action:'modifyNextAttack', params:{ stat:'dmgPct', value:0.10, mode:'add', extraStats:{ ignoreDef:0.20 }, consume:true } }
  ],
  '噬魂幼蝠': [
    { id:'night_atk', trigger:'passive', condition:{ isNight:true }, target:'self', action:'modifyStat', params:{ stat:'atkPct', value:0.15, mode:'add' } },
    { id:'night_dodge', trigger:'passive', condition:{ isNight:true }, target:'self', action:'modifyStat', params:{ stat:'dodgeRate', value:0.10, mode:'add' } },
    { id:'soul_mischief', trigger:'onAttackHit', chance:0.10, target:'target', action:'applyStatus', params:{ status:'mischief', duration:1, missChance:0.50 } }
  ],
  '赤焰幼狮': [
    { id:'mane_intimidate', trigger:'onBattleStart', target:'enemyAll', action:'applyStatus', params:{ status:'atkReduce', atkPct:-0.05, duration:2 } },
    { id:'mane_burn', trigger:'onAttackHit', target:'target', action:'applyStatus', params:{ status:'burn', duration:2, dotPct:0.05 } }
  ],
  '冰晶幼鹿': [
    { id:'crystal_refract', trigger:'onMagicAttackHit', chance:0.15, target:'enemyRandom1', action:'dealDamage', params:{ dmgPct:0.30, dmgType:'ice', powerAttr:'lastHitDmg' } }
  ],
  '玄铁幼犀': [
    { id:'iron_charge', trigger:'onFirstAttack', target:'self', action:'modifyNextAttack', params:{ stat:'dmgPct', value:0.15, mode:'add', consume:true } },
    { id:'iron_def_break', trigger:'onFirstAttack', target:'target', action:'applyStatus', params:{ status:'defReduce', defPct:-0.10, duration:1 } }
  ],
  // ===== 可进阶宠物 T3（8只）=====
  '烈焰战虎': [
    { id:'war_cry', trigger:'onBattleStart', target:'allyAll', action:'applyStatus', params:{ status:'atkBonus', atkPct:0.10, duration:2 } },
    { id:'war_kill_heal', trigger:'onKill', target:'self', action:'healPct', params:{ pct:0.05, base:'maxHp' } }
  ],
  '冰霜法师': [
    { id:'ice_mastery', trigger:'passive', target:'self', action:'modifyStat', params:{ stat:'magicDmgPct', value:0.20, mode:'add' } },
    { id:'frozen_bonus', trigger:'onMagicAttackHit', condition:{ targetHasStatus:'freeze' }, target:'self', action:'modifyStat', params:{ stat:'magicDmgPct', value:0.30, mode:'add', applyTo:'currentAttack' } }
  ],
  '暗影潜行者': [
    { id:'execute_crit', trigger:'onAttackHit', condition:{ targetHpBelow:0.25 }, target:'self', action:'forceCrit', params:{} },
    { id:'kill_speed', trigger:'onKill', target:'self', action:'applyStatus', params:{ status:'spdBonus', spdPct:0.10, duration:2 } }
  ],
  '圣盾守卫': [
    { id:'holy_shield', trigger:'onTurnStart', target:'allyLowestHp', action:'addShield', params:{ shieldType:'holyShield', absorbPct:0.08, base:'maxHp', duration:2 } }
  ],
  '雷电游侠': [
    { id:'chain_shot', trigger:'onAttackHit', chance:0.15, target:'target', action:'extraAttack', params:{ dmgPct:0.40, dmgType:'lightning', powerAttr:'atk' } },
    { id:'paralyze_crit', trigger:'onAttackHit', condition:{ targetHasStatus:'paralyze' }, target:'self', action:'forceCrit', params:{} }
  ],
  '翡翠术士': [
    { id:'jade_resonance', trigger:'onSkillCast', chance:0.20, target:'allyAll', action:'heal', params:{ healPct:0.30, base:'int' } }
  ],
  '深渊猎手': [
    { id:'pursue_bonus', trigger:'onAttackHit', condition:{ targetHpBelow:0.40 }, target:'self', action:'modifyStat', params:{ stat:'dmgPct', value:0.25, mode:'add', applyTo:'currentAttack' } },
    { id:'kill_atk_stack', trigger:'onKill', target:'self', action:'applyStatus', params:{ status:'atkBonus', atkPctPerStack:0.05, maxStack:5, stack:1 } }
  ],
  '苍穹武僧': [
    { id:'qi_wave', trigger:'onTurnStart', condition:{ everyNTurns:3 }, target:'enemyAll', action:'dealDamage', params:{ dmgPct:0.80, dmgType:'physical', powerAttr:'atk', aoe:true } },
    { id:'qi_def', trigger:'onTurnStart', condition:{ everyNTurns:3 }, target:'self', action:'applyStatus', params:{ status:'defBonus', defPct:0.15, duration:2 } }
  ]
};

// ==================== 将 PET_BLOODLINE_MECHANICS 合并到 PET_BLOOD_ALL ====================
// 关键修复：config.js 中 PET_BLOOD_ALL 构建时 mechanics 设为 []，
// 必须在此处（bloodline_engine.js 加载晚于 config.js）将 mechanics 数据回填，
// 否则 generatePetBloodlineSkill 返回的 mechanics 永远为空，所有血统机制均不生效。
(function _mergeMechanicsIntoBloodAll() {
  if (typeof PET_BLOOD_ALL === 'undefined') return;
  Object.keys(PET_BLOODLINE_MECHANICS).forEach(function(name) {
    var mechData = PET_BLOODLINE_MECHANICS[name];
    if (!mechData || mechData.length === 0) return;
    // 标准宠物：直接回填
    if (PET_BLOOD_ALL[name]) {
      PET_BLOOD_ALL[name].mechanics = mechData.map(function(m) { return JSON.parse(JSON.stringify(m)); });
    } else {
      // PET_BLOOD_ALL 中尚无此宠物条目（如融合限定宠物），创建最小条目
      PET_BLOOD_ALL[name] = {
        name: name + '之血',
        desc: '',
        effects: {},
        mechanics: mechData.map(function(m) { return JSON.parse(JSON.stringify(m)); })
      };
    }
  });
})();
