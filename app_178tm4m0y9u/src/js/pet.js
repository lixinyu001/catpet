...
// v3.x 需求1.3：旧存档迁移——无 attrPoints 时按等级自动生成，自由属性点保持未分配
if (!pet.attrPoints) {
var lv = pet.level || 1;
// 固定点每级各维+1，自由点全部保留为未分配状态
pet.attrPoints = { 力量: lv, 敏捷: lv, 体质: lv, 耐力: lv, 魔力: lv };
pet.freeAttrPoints = lv * getPetFreeAttrPointsPerLevel();
}
...
function fusePets(pet1, pet2) {
  ...
  // v3.x 需求1.3：新宠物自由属性点全部以未分配状态保留
  var _fuseLevel = 1;
  var _fuseAttrPoints = { 力量: _fuseLevel, 敏捷: _fuseLevel, 体质: _fuseLevel, 耐力: _fuseLevel, 魔力: _fuseLevel };
  var _fuseFreePts = _fuseLevel * getPetFreeAttrPointsPerLevel();
  var newPet = {
    name: basePet.name,
    race: basePet.race,
    ...
    lifespan: randomInt(10000, 15000), // 初始寿命：10000~15000
    attrPoints: _fuseAttrPoints,    // v3.x 需求1.3：固定成长属性正常生效
    freeAttrPoints: _fuseFreePts,   // v3.x 需求1.3：自由属性点保持未分配
    id: 'pet_' + Date.now() + '_' + randomInt(1000, 9999),
  };
...
function generateFusionOnlyPet(pet1, pet2) {
  ...
  var newPet = {
    ...
    petEquipment: { attack: null, hp: null, defense: null },
    lifespan: randomInt(10000, 15000), // 初始寿命：10000~15000
    attrPoints: { 力量: _foLevel, 敏捷: _foLevel, 体质: _foLevel, 耐力: _foLevel, 魔力: _foLevel },
    freeAttrPoints: _foLevel * getPetFreeAttrPointsPerLevel(),
    id: 'pet_' + Date.now() + '_' + randomInt(1000, 9999),
  };
...
function generateDivineBeast() {
  ...
  var newPet = {
    ...
    learnedSkills: [],
    level: _dbLevel, // 神兽等级同步玩家等级
    moonDewUsed: 0,
    petEquipment: { attack: null, hp: null, defense: null },
    advanceStage: 0, advanceable: false, advanceValue: 0, // 神兽不可进阶
    isDivineBeast: true,  // 神兽标记
    lifespan: 99999, // 神兽寿命无限（设为极大值）
    attrPoints: { 力量: _dbLevel, 敏捷: _dbLevel, 体质: _dbLevel, 耐力: _dbLevel, 魔力: _dbLevel },
    freeAttrPoints: _dbLevel * getPetFreeAttrPointsPerLevel(),
    id: 'pet_' + Date.now() + '_' + randomInt(1000, 9999),
  };
