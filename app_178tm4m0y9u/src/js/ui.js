...
case 'raid': app.innerHTML = (typeof renderRaidScreen === 'function') ? renderRaidScreen() : '<p>团本功能加载中...</p>'; break;
...
function renderNav() {
  ...
  return tabs.map(t => {
    ...
    // 需求4.1：任务标签可领取红点角标
    var badge = '';
    if (!isLocked && t.id === 'daily') {
      var _claimable = 0;
      if (typeof getCurrentMainQuest === 'function') {
        var _mq = getCurrentMainQuest();
        if (_mq && _mq.questData && _mq.progress >= _mq.questData.target && !_mq.claimed) _claimable++;
      }
      if (typeof DAILY_TASKS !== 'undefined') {
        DAILY_TASKS.forEach(function(task) {
          var p = G.dailyTasks[task.id] || 0;
          if (p >= task.target && !G.dailyTasks[task.id + '_claimed']) _claimable++;
        });
      }
      if (_claimable > 0) badge = '<span class="inline-block w-2 h-2 bg-red-500 rounded-full ml-0.5 animate-pulse"></span>';
    }
    return `
