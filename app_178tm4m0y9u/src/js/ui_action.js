...
function changeMap(mapId) {
  ...
  // v3.1.0 需求3.2：关卡推进式解锁检查
  if (!isMapUnlocked(mapId)) {
    showToast('该地图尚未解锁！需通关前一张地图且等级达标。', 'error');
    return;
  }
...
const PET_EQUIP_CAVE_FALLBACK = { id: 'pet_equip_cave', name: '宠物秘境', type: 'special', minLv: 20, desc: '挑战神秘秘境，获取宠物装备', ticketItem: 'pet_ticket' }; // 需求6.1：修复等级判定错误（40→20）
...
function enterSpecialDungeon(dungeonId) {
  ...
  if (G.player.level < dungeon.minLv) { showToast('等级不足！', 'error'); return false; }
  // 修复：在扣门票前检查队伍，避免无队伍时仍扣门票和记录次数导致卡住
  const team = getTeamPets();
  if (team.length === 0) { showToast('请先设置出战宠物！', 'error'); return false; }
  ...
  if (!isPetCaveDungeon) {
    ...
    if (!ticket || ticket.count <= 0) { showToast('缺少门票！', 'error'); return false; }
    ...
    if (dailyUsed >= dailyLimit) { showToast('今日该副本次数已用完（每日' + dailyLimit + '次）', 'error'); return false; }
    ticket.count--;
  ...
  return true; // 需求6.1：成功进入战斗，返回 true
}
