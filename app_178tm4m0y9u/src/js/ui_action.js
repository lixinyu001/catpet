...
// v3.x 需求1.1：静默自动刷新所有宠物属性与战力（不触发render和toast）
// 进入宠物页面时自动调用，确保展示数据为最新状态
function _autoRefreshAllPetStats() {
  if (!G.pets || G.pets.length === 0) return;
  G.pets.forEach(function(pet) {
    // 以宠物当前等级、成长、资质、属性点、技能、血统、宠物装备、符文等全部生效项为基准重新计算
    if (typeof recalcPetOnDeploy === 'function') {
      recalcPetOnDeploy(pet);
    }
    // 同步重算 rarity（基于当前成长与资质）
...
// v3.x 需求1.2：单项属性一键加满 —— 将剩余自由属性点全部分配到指定属性
function _allocateAllAttrPoints(petId, attrName) {
  var pet = G.pets.find(function(p) { return p.id === petId; });
  if (!pet) { showToast('宠物不存在', 'error'); return; }
  if (typeof getPetAttrPoints !== 'function' || typeof syncPetAttrPoints !== 'function') { showToast('属性点分配功能未加载', 'error'); return; }
  if (!pet.attrPoints) getPetAttrPoints(pet);
  syncPetAttrPoints(pet);
  var free = pet.freeAttrPoints || 0;
  if (free <= 0) { showToast('自由属性点不足', 'error'); return; }
  // 一次性将所有剩余自由属性点分配到指定属性
  pet.attrPoints[attrName] = (pet.attrPoints[attrName] || 0) + free;
  pet.freeAttrPoints = 0;
  saveGame();
  showToast('✨ ' + attrName + ' +' + free + '（已全部分配）', 'success');
  render();
}
...
if (!qty || qty <= 0) return;
// v3.x 需求2.1：边界校验——超量输入拦截
var maxAffordable = getMaxShopQty(price, currency || 'gold');
if (qty > maxAffordable) {
  showToast('数量超过可购买上限！当前最多可购买 ' + maxAffordable + ' 个', 'error');
  setShopQty(itemId, Math.max(1, maxAffordable));
  renderShopOnly();
  return;
}
