...
function renderCharacterScreen() {
  ...
  const slotsHtml = EQUIPMENT_SLOTS.map(slot => {
    ...
    var upgradeBadge = (typeof isEquipSlotUpgradeable === 'function' && isEquipSlotUpgradeable(slot.id))
      ? '<span class="absolute top-1 right-1 inline-block w-2.5 h-2.5 bg-red-500 rounded-full animate-pulse" title="有更优装备可替换"></span>'
      : '';
    return '<div class="relative bg-panel border ' + (item ? 'border-game' : 'border-dashed border-game') + ' rounded-xl p-3 text-center cursor-pointer" onclick="' + clickAction + '">' +
      upgradeBadge +
