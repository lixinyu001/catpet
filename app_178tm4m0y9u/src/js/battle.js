...
// v2.8.0 需求3.2：同关卡重复挑战的基础收益保持一致，无隐性衰减
// 奖励仅基于怪物等级和玩家转生次数，与关卡通关次数无关
// v2.10.0 需求1.2：地图等级收益梯度优化 —— 地图等级越高，单次通关基础经验/金币越高
//   mapExpMult  = 1 + (mapId-1)*0.5  → map1=1.0, map2=1.5, ... map11=6.0
//   mapGoldMult = 1 + (mapId-1)*0.4  → map1=1.0, map2=1.4, ... map11=5.0
//   确保每一档高等级地图的单位收益显著高于所有低等级地图，消除"刷低级图收益更优"的逆向激励
function giveMonsterKillRewards(monster) {
  if (!monster) return;
  // 去重：避免对同一怪物重复给奖励
  if (monster._rewarded) return;
  monster._rewarded = true;
  // v2.10.0 需求1.2：获取当前地图ID，计算地图收益梯度系数
  const mapId = (liveBattle && liveBattle.mapId) || G.player.currentMap || 1;
  const mapExpMult = 1 + (mapId - 1) * 0.5;
  const mapGoldMult = 1 + (mapId - 1) * 0.4;
  const lvDiff = monster.level - G.player.level;
  const expGain = Math.max(5, (monster.level * 8 + lvDiff * 5) * (1 + G.player.rebirth * 0.2) * mapExpMult);
  const actualExpGain = addExp(expGain);
  addBattleLog('loot', `⭐ 获得 ${actualExpGain} 经验`);
  // 主线掉落金币（无衰减，每次击杀均按此公式计算）
  const goldGain = Math.floor(monster.level * 5 * (1 + G.player.rebirth * 0.1) * mapGoldMult);
  addGold(goldGain);
