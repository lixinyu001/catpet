...
// Bug 1.1: 空数据兼容——activeTeam 或 pets 可能在异常存档中为 null/undefined
var activeTeam = (G.player && G.player.activeTeam) || [];
var pets = (G.pets && Array.isArray(G.pets)) ? G.pets : [];
// v2.8.0 需求1.1：宠物上阵时重新计算属性，确保属性点与等级匹配
var team = activeTeam.map(id => pets.find(p => p.id === id)).filter(Boolean);
if (typeof recalcPetOnDeploy === 'function') {
team.forEach(function(pet) { recalcPetOnDeploy(pet); });
}
return team;
}
...
// 需求16：怪物五维属性生成 —— 复用宠物同一套五维属性计算体系
// 按怪物定位差异化加点：物理系主加力量+耐力，法术系主加魔力+体质，高速系主加敏捷+体质
// 怪物获得比宠物更多的属性点（保持挑战度），但计算公式与宠物完全一致
function generateMonsterFiveDim(level, enemyType, race) {
  var rv = (typeof getRaceValues === 'function') ? getRaceValues(race) : { 力量:1.2, 敏捷:1.2, 体质:1.2, 耐力:1.2, 魔力:1.2 };
  // 属性点数量：怪物比宠物获得更多点数（mob 1.5x, elite 2.0x, boss 3.0x）
  var pointsPerLevel = 10; // 基础与宠物一致
  var typeMult = enemyType === 'boss' ? 3.0 : enemyType === 'elite' ? 2.0 : 1.5;
  var totalPoints = Math.floor(level * pointsPerLevel * typeMult);
  // 按种族差异化加点
  var alloc;
  if (race === '龙' || race === '恶魔') {
    // 物理系：力量+耐力
    alloc = { 力量: 0.35, 耐力: 0.25, 体质: 0.20, 敏捷: 0.20, 魔力: 0.00 };
  } else if (race === '天使' || race === '精灵') {
    // 法术系：魔力+体质
    alloc = { 魔力: 0.35, 体质: 0.25, 耐力: 0.20, 敏捷: 0.20, 力量: 0.00 };
  } else if (race === '哥布林') {
    // 高速系：敏捷+体质
    alloc = { 敏捷: 0.35, 力量: 0.25, 体质: 0.20, 耐力: 0.20, 魔力: 0.00 };
  } else {
    // 均衡型（史莱姆等）：体质为主
    alloc = { 体质: 0.30, 力量: 0.25, 耐力: 0.20, 敏捷: 0.15, 魔力: 0.10 };
  }
  // 生成属性点
  var attrPts = {};
  ATTRIBUTES.forEach(function(k) {
    attrPts[k] = Math.floor(totalPoints * (alloc[k] || 0));
  });
  // 剩余点数加到主属性
  var sumAlloc = 0;
  ATTRIBUTES.forEach(function(k) { sumAlloc += attrPts[k]; });
  var remainder = totalPoints - sumAlloc;
  if (remainder > 0) {
    var mainAttr = Object.keys(alloc).reduce(function(a, b) { return alloc[a] >= alloc[b] ? a : b; });
    attrPts[mainAttr] += remainder;
  }
  // 使用与宠物相同的转化公式
  var aptVal = 1500; // 怪物资质固定为1500（中等）
  var growth = 1.0; // 怪物成长固定为1.0
  var 力量 = 0, 敏捷 = 0, 体质 = 0, 耐力 = 0, 魔力 = 0;
  力量 = Math.round(attrPts.力量 * getAttrPointValue(aptVal, growth, rv.力量));
  敏捷 = Math.round(attrPts.敏捷 * getAttrPointValue(aptVal, growth, rv.敏捷));
  体质 = Math.round(attrPts.体质 * getAttrPointValue(aptVal, growth, rv.体质));
  耐力 = Math.round(attrPts.耐力 * getAttrPointValue(aptVal, growth, rv.耐力));
  魔力 = Math.round(attrPts.魔力 * getAttrPointValue(aptVal, growth, rv.魔力));
  // 五维→战斗属性（与宠物完全一致的公式）
  var 气血 = Math.round(体质 * 10 + level * 20);
  var 攻击力 = Math.round(力量 * 2 + 敏捷 * 0.5);
  var 防御力 = Math.round(耐力 * 1.5 + 体质 * 0.3);
  var 灵力 = Math.round(魔力 * 1.5 + 体质 * 0.3 + 耐力 * 0.3 + 力量 * 0.2);
  var 速度 = Math.round(敏捷 * 1.5);
  return {
    fiveDim: { 力量: 力量, 敏捷: 敏捷, 体质: 体质, 耐力: 耐力, 魔力: 魔力 },
    attrPts: attrPts,
    hp: 气血, atk: 攻击力, def: 防御力, spd: 速度, lingli: 灵力
  };
}
...
function getLayerMonster(mapId, forceType) {
  ...
  // Bug 2.1：怪物等级分档规则——小怪/精英/Boss 分别使用不同等级区间
  // 普通小怪：地图等级-9 ~ 地图等级-5
  // 精英怪：地图等级-4 ~ 地图等级
  // Boss：固定等于地图满等级
  var lv;
  if (enemyType === 'boss') {
    ...
    lv = randomInt(Math.max(1, map.maxLv - 4), map.maxLv);
  } else {
    lv = randomInt(Math.max(1, map.maxLv - 9), Math.max(1, map.maxLv - 5));
  }
  ...
  if (enemyType === 'boss') {
    ...
    // 需求2：精英怪也使用宠物池中的宠物名
    var elitePool = (typeof getMapMonsterPool === 'function') ? getMapMonsterPool(mapId) : map.monsters;
    name = elitePool.length > 0 ? pickRandom(elitePool) : map.eliteName;
    ...
    // 需求2：小怪使用宠物池中的T1-T4宠物名
    var mobPool = (typeof getMapMonsterPool === 'function') ? getMapMonsterPool(mapId) : map.monsters;
    name = mobPool.length > 0 ? pickRandom(mobPool) : pickRandom(map.monsters);
    race = pickRandom(mapRaces);
  ...
  // 需求16：生成怪物五维属性（复用宠物同一套五维属性计算体系）
  var monFiveDim = generateMonsterFiveDim(lv, enemyType, race);
  ...
  return {
    ...
    // 需求16：五维属性（与宠物同一套计算体系，用于血色要塞等五维加成效果）
    fiveDim: monFiveDim.fiveDim,
    fiveDimAttrPts: monFiveDim.attrPts,
    fiveDimStats: { hp: monFiveDim.hp, atk: monFiveDim.atk, def: monFiveDim.def, spd: monFiveDim.spd, lingli: monFiveDim.lingli },
  };
}

// 生成怪物波次：根据当前关卡阶段生成多个怪物
// 小怪关卡：3-6个小怪 | 精英关卡：2精英+4小怪 | Boss关卡/已通关：1boss+2精英+3小怪
// 需求7：已通关关卡（phase='cleared'）同样使用 1Boss+2精英+3小怪 阵容，修复仅剩1只Boss的问题
function generateMonsterWave(mapId) {
  ...
  } else if (prog.phase === 'boss' || prog.phase === 'cleared') {
    // Boss关卡/已通关关卡：3小怪 + 2精英 + 1Boss（Boss最后出场）
...
// v2.2.0 需求1：在线挂机彩蛋系统
// v3.x 需求3.2：移除经验/金币类奖励，彩蛋产出保底为道具类
...
{ type: 'item', itemId: 'hatch_boost', count: 1, icon: '⚡', name: '孵化加速器' },
{ type: 'item', itemId: 'forge_stone_low', count: 1, icon: '🔩', name: '低级强化石' },
];
...
function spawnMonsterDirect() {
  ...
  liveBattle = {
    ...
    // v2.11.0 需求1.3：本场战斗已使用血统复活的宠物ID集合（每场1次）
    // 修复炽天神威等神兽血统 reviveChance=1.0 导致战斗卡死的Bug
    usedBloodlineRevive: {},
    // v2.11.0 需求4.3：本场战斗已使用套装「守护者之誓」免死效果的宠物ID集合（每场1次）
    usedSetDeathImmune: {},
  };
  if (currentScreen === 'main') renderBattleArena();
  buildTurnQueue();
...
// v2.11.0 需求4.3：获取随机存活怪物（排除指定索引，用于套装溅射效果）
function getRandomAliveMonsterIdxExcluding(excludeIdxs) {
  var alive = getAliveMonsterIndices().filter(function(idx) {
    return excludeIdxs.indexOf(idx) < 0;
  });
  if (alive.length === 0) return -1;
  return alive[Math.floor(Math.random() * alive.length)];
}
...
// v3.x 需求3.1：主线地图金币、经验产出公式化
// 统一以单只怪物为基础单位，总收益随怪物数量、类型动态结算
// 单怪基础经验 = 经验基准值 × (1 + 地图等级 × 经验成长系数)
// 单怪基础金币 = 金币基准值 × (1 + 地图等级 × 金币成长系数)
// 怪物类型收益系数：普通小怪1.0 / 精英怪2.8 / Boss 9.0
function giveMonsterKillRewards(monster) {
  if (!monster) return;
  // 去重：避免对同一怪物重复给奖励
  if (monster._rewarded) return;
  monster._rewarded = true;
  // v3.x 需求3.1：以「地图等级」为核心变量的统一公式
  const mapId = (liveBattle && liveBattle.mapId) || G.player.currentMap || 1;
  // 公式参数统一配置
  const EXP_BASE = 8;        // 经验基准值
  const EXP_GROWTH = 0.5;    // 经验成长系数
  const GOLD_BASE = 5;       // 金币基准值
  const GOLD_GROWTH = 0.4;   // 金币成长系数
  // 单怪基础收益（固定，无随机波动）
  const baseExp = EXP_BASE * (1 + mapId * EXP_GROWTH);
  const baseGold = GOLD_BASE * (1 + mapId * GOLD_GROWTH);
  // 怪物类型收益系数
  const enemyType = monster.enemyType || 'mob';
  var typeMult = 1.0;
  if (enemyType === 'elite') typeMult = 2.8;       // 精英怪：2.5~3.0 → 取2.8
  else if (enemyType === 'boss') typeMult = 9.0;   // Boss：8~10 → 取9.0
  // 最终单怪收益（含转生系数，无随机波动）
  const expGain = Math.max(1, Math.floor(baseExp * typeMult * (1 + G.player.rebirth * 0.2)));
  const actualExpGain = addExp(expGain);
  addBattleLog('loot', `⭐ 获得 ${actualExpGain} 经验`);
  const goldGain = Math.max(1, Math.floor(baseGold * typeMult * (1 + G.player.rebirth * 0.1)));
  addGold(goldGain);
...
if (autoBattleInterval) maybeDropIdleEgg();
  ...
  // ===== 战斗结算界面：所有战斗类型均弹出结算Modal（仅在主界面时弹出）=====
...
if (currentScreen === 'main') render();
  ...
  // 需求：结算弹窗覆盖所有战斗类型（小怪/精英/Boss），仅在玩家在主界面时弹出
  if (currentScreen === 'main' && typeof showBattleSettlementModal === 'function') {
...
// 回合血量记录日志：记录所有宠物和怪物的当前血量
function logRoundHpStatus() {
  if (!liveBattle) return;
  var petHpList = [];
  liveBattle.team.forEach(function(pet) {
    var hp = liveBattle.petHp[pet.id];
    if (hp) {
      petHpList.push(getPetDisplayName(pet) + ':' + Math.ceil(hp.current) + '/' + hp.max);
    }
  });
  var monsterHpList = [];
  for (var i = 0; i < liveBattle.monsters.length; i++) {
    if (liveBattle.monsterHpArray[i] > 0) {
      monsterHpList.push(liveBattle.monsters[i].name + ':' + Math.ceil(liveBattle.monsterHpArray[i]) + '/' + liveBattle.monsterMaxHpArray[i]);
    }
  }
  addBattleLog('info', '📊 宠物[' + petHpList.join(', ') + '] | 怪物[' + monsterHpList.join(', ') + ']');
}
...
function executeTurn() {
  ...
  if (liveBattle.turnQueue.length === 0) {
    // ===== 回合结束血量记录日志 =====
    if (liveBattle.round > 0) {
      var endPetHp = [], endMonHp = [];
      liveBattle.team.forEach(function(p) {
        var h = liveBattle.petHp[p.id];
        if (h) endPetHp.push(getPetDisplayName(p) + ':' + Math.ceil(h.current) + '/' + h.max);
      });
      for (var mi = 0; mi < liveBattle.monsters.length; mi++) {
        if (liveBattle.monsterHpArray[mi] > 0)
          endMonHp.push(liveBattle.monsters[mi].name + ':' + Math.ceil(liveBattle.monsterHpArray[mi]) + '/' + liveBattle.monsterMaxHpArray[mi]);
      }
      addBattleLog('info', '📋 回合结束血量 — 宠物[' + endPetHp.join(', ') + '] | 怪物[' + endMonHp.join(', ') + ']');
    }
    applyTurnRegen();
    // ===== BUG修复：DoT 后先检查宠物全灭（避免残血宠物被 DoT 击杀后仍继续战斗） =====
...
function tryUseActiveSkill(pet) {
  ...
  // v2.11.0 需求1.2：辅助类技能触发概率在原有数值基础上提升100%（翻倍）
  // 辅助基础25%→50%，上限50%→100%；法术/物理不变
  var triggeredSkills = [];
  // 限制最多3个技能生效
  ...
  for (var i = 0; i < checkList.length; i++) {
    ...
    var baseChance, maxChance;
    if (isSupport) {
      baseChance = 0.50; // v2.11.0 需求1.2：辅助基础翻倍 25%→50%
      maxChance = 1.00;  // v2.11.0 需求1.2：上限翻倍 50%→100%
    } else if (isMagic) {
    ...
    // 应用上限
    triggerChance = Math.min(triggerChance, maxChance);
...
function executeAuraSkill(pet, skill) {
  ...
  if (currentScreen === 'main') {
    ...
    if (petEl) {
      // Stage 1: 前摇
      playCastWindup(petEl, false);
      petEl.classList.remove('animate-pet-attack');
      void petEl.offsetWidth;
      petEl.classList.add('animate-pet-attack');
      // Stage 2: VFX
      var vfxClassAura = getSkillVfxClass(skill);
      setTimeout(function() {
        petEl.classList.remove(vfxClassAura); void petEl.offsetWidth;
        petEl.classList.add(vfxClassAura);
      }, getAnimDelay(200));
    }
  }
}
...
function getPetAtkPower(pet, stats, passives, auras, buffs, bloodline) {
  ...
  if (bloodline && bloodline.effects) {
    ...
    if (eff.lowHpAtkBoost) {
      const hp = liveBattle.petHp[pet.id];
      const hpPct = (hp && hp.max > 0) ? hp.current / hp.max : 1;
...
function executeNormalAttack(pet) {
  ...
  // Stage 1: 普攻前摇
  if (currentScreen === 'main') {
    var _petEl = document.getElementById('battle-pet-' + pet.id);
    playCastWindup(_petEl, false);
  }
  ...
  if (mDodgeChanceAtk > 0 && Math.random() < mDodgeChanceAtk) {
    ...
    setTimeout(() => nextFromQueue(), getAnimDelay(600));
    return;
  }
  ...
  // v2.11.0 需求4.3：套装「兽王之心」追加普攻概率
  const isSetExtraAtk = stats.setExtraAttack > 0 && Math.random() < stats.setExtraAttack;

  liveBattle.monsterHpArray[targetIdx] -= damage;
  liveBattle.totalDamage += damage;
  // v2.11.0 需求4.3：套装「疾风之翼」普攻额外目标（溅射）
  if (stats.setExtraTarget > 0 && damage > 0) {
    var _extraTargetCount = Math.min(stats.setExtraTarget, 3);
    var _splashDmg = Math.floor(damage * 0.6);
    for (var _et = 0; _et < _extraTargetCount; _et++) {
      var _extraIdx = getRandomAliveMonsterIdxExcluding([targetIdx]);
      if (_extraIdx < 0) break;
      liveBattle.monsterHpArray[_extraIdx] -= _splashDmg;
      liveBattle.totalDamage += _splashDmg;
      var _extraMon = liveBattle.monsters[_extraIdx];
      addBattleLog('skill', `🌪️ ${getPetDisplayName(pet)} 疾风溅射对 ${_extraMon.name} 造成 ${_splashDmg} 点伤害`);
      if (currentScreen === 'main') animatePetAttack(pet, { val: _splashDmg, crit: false }, _extraIdx);
      if (liveBattle.monsterHpArray[_extraIdx] <= 0) {
        liveBattle.monsterHpArray[_extraIdx] = 0;
        logMonsterKill(_extraIdx);
        if (currentScreen === 'main') animateMonsterDeath(_extraIdx);
      }
    }
  }
  ...
  } else if (isSetExtraAtk) {
    // v2.11.0 需求4.3：套装「兽王之心」追加普攻
    setTimeout(() => {
      const dmg2 = Math.max(1, Math.floor(damage * 0.8));
      liveBattle.monsterHpArray[targetIdx] -= dmg2;
      liveBattle.totalDamage += dmg2;
      addBattleLog('skill', `🦁 ${getPetDisplayName(pet)} 套装【兽王之心】追加普攻！对 ${monster.name} 造成 ${dmg2} 点伤害`);
      if (currentScreen === 'main') animatePetAttack(pet, { val: dmg2, crit: false }, targetIdx);
      if (liveBattle.monsterHpArray[targetIdx] <= 0) {
        liveBattle.monsterHpArray[targetIdx] = 0;
        logMonsterKill(targetIdx);
...
function executeActiveSkill(pet, skill) {
  ...
  if (currentScreen === 'main') {
    var _petEl = document.getElementById('battle-pet-' + pet.id);
    var _isMagic = !isPhysicalSkill(skill);
    playCastWindup(_petEl, _isMagic);
  }
  const stats = getPetStats(pet);
  const passives = getPassiveEffects(pet);
  const auras = getAuraEffects(liveBattle.team);
  ...
  // 技能额外无视防御（血统修改器 + 被动无视防御）
  // 需求8：passives.ignoreDef 被动效果对主动技能同样生效
  var _skBlIgnoreDef = 0;
  if (passives.ignoreDef) _skBlIgnoreDef += passives.ignoreDef;
  ...
  if (category === 'single_atk' || category === 'aoe_atk' || category === 'single_cc' || category === 'aoe_cc') {
    ...
    // v2.11.0 需求4.3：套装「智慧之渊」法术额外目标（仅法术技能生效）
    if (isMagic && stats.setMagicExtraTarget > 0 && !isAOE) {
      var _magicExtraCount = Math.min(stats.setMagicExtraTarget, 2);
      for (var _me = 0; _me < _magicExtraCount; _me++) {
        var _magicExtraIdx = getRandomAliveMonsterIdxExcluding(targetIndices);
        if (_magicExtraIdx < 0) break;
        targetIndices.push(_magicExtraIdx);
      }
      if (_magicExtraCount > 0) {
        addBattleLog('skill', `📘 ${getPetDisplayName(pet)} 套装【智慧之渊】法术扩散！目标+${targetIndices.length - 1}`);
      }
    }
    ...
    // 需求8：全队光环暴击率加成（被动效果对主动技能同样生效）
    if (auras.teamCrit) critChance += auras.teamCrit;
    // 需求8：血统通用效果暴击率加成（被动效果对主动技能同样生效）
    if (bloodline && bloodline.effects && bloodline.effects.critRate) critChance += bloodline.effects.critRate;
    // 人物暴击率20%附加给宠物
    if (stats.critRate) critChance += stats.critRate;
    ...
    // 需求3-BUG修复：追踪每个目标的实际伤害与状态（闪避/格挡/免疫），用于飘字渲染
    var skillDamageMap = {};
    ...
    targetIndices.forEach(function(idx) {
      ...
      // 需求3-BUG修复：记录该目标的实际伤害与状态
      skillDamageMap[idx] = { val: dmg, crit: isCrit, parried: _parried };
      if (skill.hits) {
        var _totalHitDmg = 0;
        ...
        skillDamageMap[idx].val = _totalHitDmg;
      } else {
  ...
  if (isMagic && (category === 'single_atk' || category === 'aoe_atk' || category === 'single_cc' || category === 'aoe_cc') && passives.skillDouble && magicDoubleTargets.length > 0) {
    if (Math.random() < passives.skillDouble) {
      ...
      magicDoubleTargets.forEach(function(idx) {
        ...
        // 独立判定暴击（需求8：被动效果对主动技能同样生效）
        var fCritChance = 0.10 + (passives.critRate || 0) + (skill.bonusCrit || 0) + (auras.teamCrit || 0);
        if (bloodline && bloodline.effects && bloodline.effects.critRate) fCritChance += bloodline.effects.critRate;
        fCritChance += (stats.critRate || 0);
  ...
  if (currentScreen === 'main') {
    ...
    if (isHeal || isBuff) {
      ...
      if (petEl) {
        // Stage 2: VFX 特效爆发
        var vfxClassHeal = getSkillVfxClass(skill);
        petEl.classList.remove(vfxClassHeal); void petEl.offsetWidth;
        petEl.classList.add(vfxClassHeal);
        spawnDamageFloat(petEl, { val: 0, heal: true });
      }
      ...
      targetIndices.forEach(function(idx) {
        ...
        if (dmgInfo && dmgInfo.dodge) {
          // 闪避：弹出闪避文本
          animateSkillAttack(pet, skill, { dodge: true }, idx);
        } else if (dmgInfo && dmgInfo.parried) {
          // 格挡：弹出格挡文本 + 实际伤害
          animateSkillAttack(pet, skill, { val: dmgInfo.val, crit: false, parried: true }, idx);
        } else if (dmgInfo) {
          // 正常命中：弹出真实伤害数值
          animateSkillAttack(pet, skill, { val: dmgInfo.val, crit: dmgInfo.crit }, idx);
        }
      });
      // 对已死亡怪物播放死亡动画
      liveBattle.monsters.forEach(function(m, i) {
        if (liveBattle.monsterHpArray[i] <= 0) animateMonsterDeath(i);
      });
    }
  }
...
function executeMonsterSkill(monsterIdx, skill) {
  ...
  // Stage 1: 怪物施法前摇
  if (currentScreen === 'main') {
    var _monEl = getMonsterEl(monsterIdx);
    var _isMagicM = !isPhysicalSkill(skill);
    playCastWindup(_monEl, _isMagicM);
  }
  var isMagic = !isPhysicalSkill(skill);
  ...
  if (category === 'single_atk' || category === 'aoe_atk' || category === 'single_cc' || category === 'aoe_cc') {
    ...
    targetPets.forEach(function(tp) {
      ...
      // v2.11.0 需求4.3：套装「守护者之誓」免死效果（每场战斗1次，怪物技能攻击场景）
      if (liveBattle.petHp[tp.id].current <= 0
          && tStats.setDeathImmune
          && !liveBattle.usedSetDeathImmune[tp.id]) {
        liveBattle.petHp[tp.id].current = 1;
        liveBattle.usedSetDeathImmune[tp.id] = true;
        addBattleLog('skill', `🛡️ ${getPetDisplayName(tp)} 套装【守护者之誓】免死触发！保留1点气血`);
      }
      totalDamage += dmg;
      dmgPerPet[tp.id] = dmg;
      // ===== 血统机制：受击触发（怪物技能攻击） =====
    ...
    // 检查宠物死亡（含复活/血统判定，与 executeMonsterAction 保持一致）
    targetPets.forEach(function(tp) {
      if (liveBattle.petHp[tp.id].current <= 0) {
        // ===== 血统机制：致命伤害触发（在死亡判定前） =====
        if (typeof triggerBloodlineMechanics === 'function') {
          triggerBloodlineMechanics(tp, 'onFatalDamage', {
            attacker: monster, monsterIdx: monsterIdx,
            damage: dmgPerPet[tp.id] || 0
          });
          if (liveBattle.petHp[tp.id].current > 0) return; // 被血统技能救活
        }
        var tPassives2 = getPassiveEffects(tp);
        var tBloodline2 = getBloodlineSkill(tp);
        var revived = false;
        if (tPassives2.reviveChance && Math.random() < tPassives2.reviveChance) {
          liveBattle.petHp[tp.id].current = Math.floor(liveBattle.petHp[tp.id].max * tPassives2.revivePct);
          addBattleLog('heal', `✨ ${getPetDisplayName(tp)} 神佑复活！恢复 ${Math.floor(tPassives2.revivePct*100)}% 气血`);
          revived = true;
        }
        // v2.11.0 需求1.3：血统复活每场战斗限1次（修复 reviveChance=1.0 导致的战斗卡死Bug）
        if (!revived && tBloodline2 && tBloodline2.effects && tBloodline2.effects.reviveChance
            && !liveBattle.usedBloodlineRevive[tp.id]
            && Math.random() < tBloodline2.effects.reviveChance) {
          var revivePct2 = tBloodline2.effects.reviveHpPct || 0.30;
          liveBattle.petHp[tp.id].current = Math.floor(liveBattle.petHp[tp.id].max * revivePct2);
          addBattleLog('heal', `🟢 ${getPetDisplayName(tp)} 血统复活！恢复 ${Math.floor(revivePct2*100)}% 气血`);
          liveBattle.usedBloodlineRevive[tp.id] = true; // 标记本场已使用
          revived = true;
        }
        if (!revived) {
          addBattleLog('info', `💔 ${getPetDisplayName(tp)} 阵亡了！`);
          if (typeof triggerBloodlineMechanics === 'function') {
            triggerBloodlineMechanics(tp, 'onDeath', { killer: monster, monsterIdx: monsterIdx });
            liveBattle.team.forEach(function(ally) {
              if (ally.id === tp.id) return;
              var allyHp = liveBattle.petHp[ally.id];
              if (!allyHp || allyHp.current <= 0) return;
              triggerBloodlineMechanics(ally, 'onAllyDeath', { deadAlly: tp, killer: monster, monsterIdx: monsterIdx });
            });
          }
        }
      }
  ...
  setTimeout(function() { nextFromQueue(); }, getAnimDelay(600));
}
...
// v2.11.0 需求4.3：套装「守护者之誓」免死效果（每场战斗1次）
if (liveBattle.petHp[targetPet.id].current <= 0
    && targetStats.setDeathImmune
    && !liveBattle.usedSetDeathImmune[targetPet.id]) {
  liveBattle.petHp[targetPet.id].current = 1;
  liveBattle.usedSetDeathImmune[targetPet.id] = true;
  addBattleLog('skill', `🛡️ ${getPetDisplayName(targetPet)} 套装【守护者之誓】免死触发！保留1点气血`);
}
// 寿命系统：检测宠物阵亡
if (liveBattle.petHp[targetPet.id].current <= 0) {
...
liveBattle.logs.unshift({ type: 'damage', msg: `👹 ${monster.name} 普攻 ${getPetDisplayName(targetPet)} 造成 ${monsterDmg} 点伤害` });
    ...
    if (liveBattle.petHp[targetPet.id].current <= 0) {
      ...
      // v2.11.0 需求1.3：使用 liveBattle.usedBloodlineRevive 标记持久化，修复原 Object.assign 仅作用于局部变量的Bug
      if (!revived && bloodline && bloodline.effects && bloodline.effects.reviveChance
          && !liveBattle.usedBloodlineRevive[targetPet.id]
          && Math.random() < bloodline.effects.reviveChance) {
        ...
        liveBattle.usedBloodlineRevive[targetPet.id] = true; // 标记本场已使用
        revived = true;
      }
      if (!revived) {
  ...
  // ===== BUG修复：宠物死亡后即时检查全灭，避免残血宠物被跳过回合直接判负 =====
  var allPetsDead = liveBattle.team.every(function(p) {
    var hp = liveBattle.petHp[p.id];
    return !hp || hp.current <= 0;
  });
  if (allPetsDead) {
...
function applyTurnRegen() {
  ...
  liveBattle.team.forEach(pet => {
    ...
    // v2.11.0 需求4.3：套装「生机之息」回合回血效果
    const peStats = (typeof getPetStats === 'function') ? getPetStats(pet) : null;
    if (peStats && peStats.setEndRegen) regenPct += peStats.setEndRegen;
    if (regenPct > 0) {
    ...
    // ===== BUG修复：DoT 击杀宠物时给予复活/血统判定，避免残血宠物被跳过 =====
    if (hp.current <= 0) {
      // v2.11.0 需求4.3：套装「守护者之誓」免死效果（DoT击杀场景，每场战斗1次）
      var _dotPeStats = (typeof getPetStats === 'function') ? getPetStats(pet) : null;
      if (_dotPeStats && _dotPeStats.setDeathImmune && !liveBattle.usedSetDeathImmune[pet.id]) {
        hp.current = 1;
        liveBattle.usedSetDeathImmune[pet.id] = true;
        addBattleLog('skill', `🛡️ ${getPetDisplayName(pet)} 套装【守护者之誓】免死触发！保留1点气血`);
        return;
      }
      if (typeof triggerBloodlineMechanics === 'function') {
        triggerBloodlineMechanics(pet, 'onFatalDamage', { attacker: null, monsterIdx: -1, damage: 0 });
        if (hp.current > 0) return; // 被血统技能救活
      }
      var dotRevived = false;
      if (passives.reviveChance && Math.random() < passives.reviveChance) {
        hp.current = Math.floor(hp.max * passives.revivePct);
        addBattleLog('heal', `✨ ${getPetDisplayName(pet)} 神佑复活！恢复 ${Math.floor(passives.revivePct*100)}% 气血`);
        dotRevived = true;
      }
      // v2.11.0 需求1.3：血统复活每场战斗限1次（DoT击杀场景同步修复）
      if (!dotRevived && bloodline && bloodline.effects && bloodline.effects.reviveChance
          && !liveBattle.usedBloodlineRevive[pet.id]
          && Math.random() < bloodline.effects.reviveChance) {
        var dotRevivePct = bloodline.effects.reviveHpPct || 0.30;
        hp.current = Math.floor(hp.max * dotRevivePct);
        addBattleLog('heal', `🟢 ${getPetDisplayName(pet)} 血统复活！恢复 ${Math.floor(dotRevivePct*100)}% 气血`);
        liveBattle.usedBloodlineRevive[pet.id] = true; // 标记本场已使用
        dotRevived = true;
      }
      if (!dotRevived) {
        addBattleLog('info', `💔 ${getPetDisplayName(pet)} 被 DoT 效击败身亡！`);
      }
    }
    if (buffs) {
...
function processBattleDefeat() {
  if (!liveBattle) return;
  // 符文洞窟战斗失败：不扣除挑战次数，不损失经验（v2.8.0原进化森林）
  if (liveBattle.isEvolutionForest) {
    addBattleLog('info', '🔮 符文洞窟挑战失败，挑战次数不扣除，再接再厉！');
    G.battleContext = null;
    saveGame();
    if (typeof cleanupBloodlineRT === 'function') cleanupBloodlineRT();
    liveBattle = null;
    if (currentScreen === 'main') render();
    if (currentScreen === 'activity') render();
    showToast('符文洞窟挑战失败，次数不扣除', 'error');
    return;
  }
...
function processTreasureRewards() {
  ...
  if (Math.random() < eggDrop || (tmap.special && tmap.special.id === 'guaranteed_egg')) {
    ...
    if (tmap.mapType && typeof getTreasureMapPetPool === 'function') {
      ...
      // 需求17：T5阶蛋主线不掉落，cap at 3(T4)；新手保护同步生效
      var _eggTierBase = (typeof getDropEggTier === 'function') ? getDropEggTier() : getEggTierByMapLevel();
      var tier = tmap.special && tmap.special.id === 'guaranteed_egg' ? Math.min(3, _eggTierBase + 1) : _eggTierBase;
    ...
    G.player.activeTeam.forEach(function(petId) {
      ...
      if (pet && pet.level < G.player.level) {
        ...
        while (pet.exp >= needed && pet.level < G.player.level) {
          pet.exp -= needed;
          pet.level++;
          // v2.11.0 需求3.2：升级时发放属性点 + 按保存的自动加点方案分配自由属性点
          if (typeof grantAttrPointsOnLevelUp === 'function') {
            grantAttrPointsOnLevelUp(pet);
          } else {
            // 兼容回退：每级固定+1五维，自由点+5
            if (typeof ATTRIBUTES !== 'undefined') {
              ATTRIBUTES.forEach(function(attr) { pet.attrPoints[attr] = (pet.attrPoints[attr] || 0) + 1; });
            }
            pet.freeAttrPoints = (pet.freeAttrPoints || 0) + 5;
          }
          // v2.11.0 需求3.2：自动按保存方案分配新获得的自由点
  ...
  // v2.9.0 需求2.3：藏宝图奖池新增宠物洗点丹（紫色+藏宝图5%基础掉落，橙色+10%）
  var resetPillChance = (tmap.rarity === 'orange') ? 0.10 : (tmap.rarity === 'purple' ? 0.05 : 0);
  if (Math.random() < resetPillChance) {
    var resetPillCount = isTriple ? 3 : (isDouble ? 2 : 1);
    var resetPillExisting = G.inventory.find(function(i) { return i.id === 'pet_reset_pill'; });
    if (resetPillExisting) resetPillExisting.count += resetPillCount;
    else G.inventory.push({ id: 'pet_reset_pill', count: resetPillCount });
    addBattleLog('loot', `🧪 获得宠物洗点丹 x${resetPillCount}`);
    rewards.push({ icon: '🧪', name: '宠物洗点丹', amount: resetPillCount, color: '#10b981' });
  }
  G.statistics.totalBattles++;
  updateAchievement('battle', 1);
...
// 需求9：副本倍率掉落判定 —— 按配置概率随机返回 1/3/9/27 倍率
function rollDungeonDropMultiplier() {
  if (typeof DUNGEON_DROP_MULTIPLIER === 'undefined' || !DUNGEON_DROP_MULTIPLIER.tiers) return 1;
  var tiers = DUNGEON_DROP_MULTIPLIER.tiers;
  // 从高倍率到低倍率依次判定，命中即返回
  for (var i = tiers.length - 1; i >= 0; i--) {
    if (Math.random() < tiers[i].chance) return tiers[i].mult;
  }
  return 1;
}
...
function processDungeonWaveComplete() {
  ...
  if (liveBattle.dungeonWave >= liveBattle.dungeonMaxWaves) {
    ...
    // 需求9：副本倍率掉落判定
    var dropMult = rollDungeonDropMultiplier();
    var multTag = dropMult > 1 ? ` ✨×${dropMult}倍率！` : '';
    if (dropMult > 1) {
      addBattleLog('loot', `✨ 触发副本倍率掉落 ×${dropMult}！`);
    }
    // v2.8.0 评审修复（一般问题5）：追踪结构化奖励数据，填充到 dungeonRewardInfo
    var dungeonGoldGain = 0, dungeonExpGain = 0, dungeonItemGains = [];
    let reward = '';
    if (liveBattle.dungeonId === 'exp_cave') {
      const exp = (G.player.level * 500 + randomInt(100, 500)) * dropMult;
      const actualExp = addExp(exp);
      dungeonExpGain = actualExp;
      reward = `获得 ${actualExp.toLocaleString()} 经验${multTag}`;
    } else if (liveBattle.dungeonId === 'gold_mine') {
      const gold = (G.player.level * 200 + randomInt(500, 2000)) * dropMult;
      addGold(gold);
      dungeonGoldGain = gold;
      reward = `获得 ${gold.toLocaleString()} 金币${multTag}`;
    } else if (liveBattle.dungeonId === 'egg_forest') {
      var tier = getEggTierByMapLevel();
      // 蛋之森林额外提升1级
      tier = Math.min(4, tier + 1);
      var eggCount = 1 * dropMult;
      for (var ei = 0; ei < eggCount; ei++) {
        G.eggs.push(generateEgg(tier));
      }
      reward = `获得 T${tier+1} 宠物蛋${eggCount > 1 ? ' ×' + eggCount : ''}${multTag}`;
      dungeonItemGains.push({ name: 'T' + (tier+1) + '宠物蛋', count: eggCount });
    } else if (liveBattle.dungeonId === 'skill_tower') {
      // 技能秘境：从全技能池随机掉落1~2本技能书
      const pool = ALL_SKILLS;
      const dropCount = randomInt(1, 2) * dropMult;
      var droppedNames = [];
      for (var si = 0; si < dropCount; si++) {
        ...
        if (droppedNames.indexOf(skill.name) === -1) droppedNames.push(skill.name);
      }
      reward = `获得技能书 ×${dropCount}：${droppedNames.join('、')}${multTag}`;
      dungeonItemGains.push({ name: '技能书：' + droppedNames.join('、'), count: dropCount });
      ...
      const stoneCount = randomInt(2, 4) * dropMult;
      ...
      reward = `获得 ${stoneName} ×${stoneCount}${multTag}`;
      dungeonItemGains.push({ name: stoneName, count: stoneCount });
      ...
      const mapCount = randomInt(1, 2) * dropMult;
      ...
      reward = `获得 ${mapName} ×${mapCount}${multTag}`;
      dungeonItemGains.push({ name: mapName, count: mapCount });
      ...
      var totalGemDrops = (baseDropCount + bonusDrops) * dropMult;
      ...
      for (var gj = 0; gj < totalGemDrops; gj++) {
        ...
        var gemLabel = gemDef.name + '+' + gemLevel;
        if (droppedGemNames.indexOf(gemLabel) === -1) droppedGemNames.push(gemLabel);
      }
      reward = `获得宝石 ×${totalGemDrops}：${droppedGemNames.join('、')}${multTag}`;
      dungeonItemGains.push({ name: '宝石：' + droppedGemNames.join('、'), count: totalGemDrops });
      ...
      // 需求9：倍率掉落——生成 dropMult 件宠物装备
      var peDropCount = 1 * dropMult;
      var peEquipNames = [];
      for (var pei = 0; pei < peDropCount; pei++) {
        var peEquip = generatePetEquip(peRarity);
        addPetEquipToBag(peEquip);
        if (peEquipNames.indexOf(peEquip.name) === -1) peEquipNames.push(peEquip.name);
      }
      reward = `获得 ${PET_EQUIP_RARITY_NAMES[peRarityIdx]} ${peEquipNames.join('、')}${peDropCount > 1 ? ' ×' + peDropCount : ''}${multTag}`;
      dungeonItemGains.push({ name: PET_EQUIP_RARITY_NAMES[peRarityIdx] + ' ' + peEquipNames.join('、'), count: peDropCount });
      // 需求6：宠物秘境收获日志
      if (typeof addActivityLog === 'function') {
        addActivityLog('petcave', '通关宠物秘境（' + waveCount + '波），获得 ' + PET_EQUIP_RARITY_NAMES[peRarityIdx] + ' ' + peEquipNames.join('、') + (peDropCount > 1 ? ' ×' + peDropCount : '') + multTag, 'win');
      }
      // 需求2：30%概率额外掉落材料（按波次决定材料等级）
      // v2.10.0 需求1.1：战兵图册与神秘水晶产出比例严格1:1，同时掉落等量两类材料
      // v2.11.0 需求2.1：远古符文（宠装）→ 战兵图册
      if (Math.random() < 0.30) {
        // 波次越多，材料等级越高
        var matGradePool;
        if (waveCount >= 5) matGradePool = ['mid','high'];
        else if (waveCount >= 3) matGradePool = ['low','mid'];
        else matGradePool = ['low'];
        var matGrade = matGradePool[randomInt(0, matGradePool.length - 1)];
        // v2.10.0 需求1.1：同时掉落等量的神秘水晶和战兵图册，确保比例严格1:1
        var crystalKey = 'mystic_crystal_' + matGrade;
        var warBookKey = 'war_book_' + matGrade;
        // 需求9：材料也受倍率影响
        var matCount = randomInt(1, 3) * dropMult;
        G.petEquipMaterials[crystalKey] = (G.petEquipMaterials[crystalKey] || 0) + matCount;
        G.petEquipMaterials[warBookKey] = (G.petEquipMaterials[warBookKey] || 0) + matCount;
        reward += '，' + PET_EQUIP_MATERIAL_NAMES[crystalKey] + ' x' + matCount + '，' + PET_EQUIP_MATERIAL_NAMES[warBookKey] + ' x' + matCount;
        if (typeof addActivityLog === 'function') {
          addActivityLog('petcave', '额外掉落 ' + PET_EQUIP_MATERIAL_NAMES[crystalKey] + ' x' + matCount + '，' + PET_EQUIP_MATERIAL_NAMES[warBookKey] + ' x' + matCount, 'win');
        }
      }
    } else if (liveBattle.dungeonId === 'bloodline_dungeon') {
      // 需求5：血统副本通关奖励（血统珠已在每波掉落逻辑中处理，这里给额外金币和经验）
      var bdGold = (G.player.level * 100 + randomInt(200, 800)) * dropMult;
      addGold(bdGold);
      dungeonGoldGain = bdGold;
      reward = `获得 ${bdGold.toLocaleString()} 金币（血统珠已在波次中掉落）${multTag}`;
    }
    ...
    saveGame();
    // v2.8.0 需求3.1：副本结算展示暴击标识与奖励明细
    // v2.8.0 评审修复（一般问题5）：填充结构化奖励数据（gold/exp/items）
    var dungeonRewardInfo = {
      gold: dungeonGoldGain,
      exp: dungeonExpGain,
      items: dungeonItemGains,
      dropMult: dropMult,
      dungeonName: dungeon ? dungeon.name : '副本',
      isDungeon: true,
    };
    // 解析 reward 文本提取奖励项
    if (dropMult > 1) {
      dungeonRewardInfo.critTag = '✨ ×' + dropMult + ' 倍率掉落！';
    }
    liveBattle = null;
    if (currentScreen === 'main') render();
    showToast(`副本完成！${reward}`, 'success');
    // v2.8.0 需求3.1：副本结算弹窗展示暴击与奖励明细
    if (currentScreen === 'main' && typeof showDungeonSettlementModal === 'function') {
      showDungeonSettlementModal(dungeonRewardInfo, reward);
    }
...
function maybeDropIdleEgg() {
  ...
  var amount = 0;
  // v3.x 需求3.2：移除gold/exp奖励分支，仅保留道具和钻石
  ...
  // 彩蛋统计
  // v3.x 需求3.2：移除gold/exp统计字段
  if (!G.idleEggStats) G.idleEggStats = { total: 0, diamond: 0, items: 0 };
  G.idleEggStats.total++;
  if (reward.type === 'diamond') G.idleEggStats.diamond++;
...
// ===== 三段式动画辅助函数 =====
// 获取按战斗速度缩放的动画延迟
function getAnimDelay(baseMs) {
  var mult = (G.battleSpeed || 1) * 1.5;
  return Math.max(20, Math.floor(baseMs / mult));
}

// 获取技能元素对应的VFX特效类名
function getSkillVfxClass(skill) {
  if (!skill || !skill.element) return 'animate-vfx-neutral';
  var map = {
    '火': 'animate-vfx-fire', '冰': 'animate-vfx-ice', '雷': 'animate-vfx-thunder',
    '暗': 'animate-vfx-shadow', '光': 'animate-vfx-heal', '毒': 'animate-vfx-poison'
  };
  return map[skill.element] || 'animate-vfx-neutral';
}

// Stage 1：播放施法前摇动画
function playCastWindup(casterEl, isMagic) {
  if (!casterEl) return;
  var cls = isMagic ? 'animate-cast-windup-magic' : 'animate-cast-windup';
  casterEl.classList.remove(cls);
...
function animatePetAttack(pet, dmg, monsterIdx) {
  ...
  setTimeout(() => {
    if (monsterEl) {
      monsterEl.classList.remove('animate-monster-hurt-enhanced'); void monsterEl.offsetWidth;
      monsterEl.classList.add('animate-monster-hurt-enhanced');
    }
...
// 技能三段式动画（前摇→特效爆发→受击反馈）
function animateSkillAttack(pet, skill, dmg, monsterIdx) {
  const petEl = document.getElementById('battle-pet-' + pet.id);
  const monsterEl = getMonsterEl(monsterIdx);
  const vfxClass = getSkillVfxClass(skill);
  var isDodge = dmg && dmg.dodge;
  // Stage 1: 施法前摇（已在 executeActiveSkill 中通过 playCastWindup 播放）
  if (petEl) { petEl.classList.remove('animate-pet-attack'); void petEl.offsetWidth; petEl.classList.add('animate-pet-attack'); }
  // Stage 2 + 3: 特效爆发 + 受击反馈
  setTimeout(() => {
    if (monsterEl) {
      if (!isDodge) {
        // Stage 2: VFX 特效爆发
        monsterEl.classList.remove(vfxClass); void monsterEl.offsetWidth;
        monsterEl.classList.add(vfxClass);
        // Stage 3: 增强版受击反馈
        monsterEl.classList.remove('animate-monster-hurt-enhanced'); void monsterEl.offsetWidth;
        monsterEl.classList.add('animate-monster-hurt-enhanced');
      } else {
...
function animateMonsterAttackAnim(targetPet, dmg, monsterIdx) {
  ...
  // Stage 2 + 3: 宠物受击反馈 + 伤害飘字
  setTimeout(() => {
    if (petEl) {
      petEl.classList.remove('animate-pet-hurt-enhanced'); void petEl.offsetWidth;
      petEl.classList.add('animate-pet-hurt-enhanced');
    }
...
function spawnDamageFloat(parentEl, dmg) {
  ...
  // 需求3-BUG修复：闪避/格挡/免疫显示对应文本而非0伤害
  if (dmg.dodge) {
    floatEl.className = 'dodge-float';
  ...
  if (dmg.immune) {
    floatEl.className = 'immune-float';
    floatEl.textContent = '免疫';
    floatEl.style.cssText = 'position:absolute;font-weight:900;pointer-events:none;z-index:10;color:#a78bfa;font-size:1rem;left:50%;top:30%;animation:dodgeFloat 0.8s ease-out forwards;';
    parentEl.style.position = 'relative';
    parentEl.appendChild(floatEl);
    setTimeout(() => floatEl.remove(), 800);
    return;
  ...
  if (dmg.heal) {
    ...
    // 格挡：显示「格挡」+ 实际伤害值
    floatEl.className = 'parry-float';
    floatEl.textContent = '格挡 -' + dmg.val;
    floatEl.style.cssText = 'position:absolute;font-weight:900;pointer-events:none;z-index:10;color:#60a5fa;font-size:1rem;left:50%;top:25%;animation:damageFloat 0.9s ease-out forwards;';
  } else {
